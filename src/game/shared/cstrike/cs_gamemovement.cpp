...
unsigned int CCSGameMovement::PlayerSolidMask( bool brushOnly )
{
	...
	if (m_pCSPlayer)
	{
		switch (m_pCSPlayer->GetTeamNumber())
		{
		case TEAM_SURVIVOR:
	...
	if ( brushOnly )
	{
		if ( isBot )
		{
			return (uMask | MASK_PLAYERSOLID_BRUSHONLY | CONTENTS_MONSTERCLIP);
		}
		else
		{
			return (uMask | MASK_PLAYERSOLID_BRUSHONLY);
		}
	}
	else
	{
		if ( isBot )
		{
			return (uMask | MASK_PLAYERSOLID | CONTENTS_MONSTERCLIP);
		}
		else
		{
...
{
	QAngle	v_angle;
	mv->m_flMaxSpeed = m_pCSPlayer->GetPlayerMaxSpeed();

...
BEGIN_PREDICTION_DATA( CWeaponCSBase )
	...
	DEFINE_PRED_FIELD( m_flNextAttack3, FIELD_FLOAT, 0 ),
...
CWeaponCSBase::CWeaponCSBase()
{
	...
	m_flNextAttack3 = 0.0f;

#ifdef CLIENT_DLL
	m_iCrosshairTextureID = 0;
#else
	m_iDefaultExtraAmmo = 0;
	m_hMeleeTraceHitEnt = NULL;
	m_vecMeleeForward.Init();
#endif
...
void CWeaponCSBase::SecondaryAttack( void )
{
	...
	const float smackDelay = 0.1f;
	const float range = 48.0f;

	m_flNextSecondaryAttack = gpGlobals->curtime + cooldown;
	m_flNextPrimaryAttack = gpGlobals->curtime + cooldown;
	SetWeaponIdleTime( gpGlobals->curtime + cooldown );

	SendWeaponAnim( ACT_VM_MELEE_LAYER );
	FX_PlantBomb(pPlayer->entindex(), pPlayer->Weapon_ShootPosition(), PLANTBOMB_PLANT);

	// Swing sound always plays, even on hit.
	{
		CPASAttenuationFilter filter( this );
		filter.UsePredictionRules();
		EmitSound( filter, entindex(), "Weapon.Swing" );
	}

#ifndef CLIENT_DLL
	Vector forward;
	AngleVectors( pPlayer->EyeAngles(), &forward );
	m_vecMeleeForward = forward;

	Vector vecSrc = pPlayer->Weapon_ShootPosition();
	Vector vecEnd = vecSrc + forward * range;

	trace_t tr;
	UTIL_TraceLine( vecSrc, vecEnd, MASK_SOLID, pPlayer, COLLISION_GROUP_NONE, &tr );
	if ( tr.fraction >= 1.0f )
	{
		const Vector hullMins( -16, -16, -18 );
		const Vector hullMaxs( 16, 16, 18 );
		UTIL_TraceHull( vecSrc, vecEnd, hullMins, hullMaxs, MASK_SOLID, pPlayer, COLLISION_GROUP_NONE, &tr );
	}

	m_meleeTrace = tr;
	m_hMeleeTraceHitEnt = tr.m_pEnt;
	SetContextThink( &CWeaponCSBase::MeleeSmack, gpGlobals->curtime + smackDelay, "CSWeaponMeleeSmack" );
#endif
}

#ifndef CLIENT_DLL
void CWeaponCSBase::MeleeSmack()
{
	CCSPlayer* pPlayer = GetPlayerOwner();
	if (!pPlayer)
		return;
	...
	int count = UTIL_EntitiesInSphere(pList, 64, src, range, FL_CLIENT | FL_NPC);

	for (int i = 0; i < count; i++)
	{
		CBaseEntity* pHit = pList[i];
		...
		const bool hitCombatCharacter = pHit->IsPlayer() || pHit->IsNPC();

		if (hitCombatCharacter)
		{
			const bool isFriendly =
				pHit->IsPlayer() &&
				(pHit->GetTeamNumber() == pPlayer->GetTeamNumber());

			EmitSound(isFriendly ? "Weapon.HitSurvivor" : "Weapon.HitInfected");
		}
		else
		{
			EmitSound("Weapon.HitWorld");
		}

		if (pHit->m_takedamage == DAMAGE_NO)
			continue;

		Vector attackDir = (pHit->WorldSpaceCenter() - src);
		attackDir.NormalizeInPlace();

		ClearMultiDamage();

		CTakeDamageInfo info(pPlayer, pPlayer, damage, DMG_CLUB);
		...
		pHit->DispatchTraceAttack(info, attackDir, &tr);
...
bool CWeaponCSBase::SendWeaponAnim( int iActivity )
{
	...
	if ( pPlayer && pPlayer->HasShield() )
	{
		...
		int	idealSequence = SelectWeightedSequenceForViewModel( vm, (Activity)iActivity );
...
bool CWeaponCSBase::DefaultDeploy( char *szViewModel, char *szWeaponModel, int iActivity, char *szAnimExt )
{
	...
	SendWeaponAnim(ACT_VM_DEPLOY_LAYER);
	pOwner->SetNextAttack(gpGlobals->curtime + SequenceDuration(SelectWeightedSequence(ACT_VM_DEPLOY_LAYER)));
...
float CalcViewModelBobHelper(CBasePlayer* player, BobState_t* pBobState, int nVMIndex)
{
	...
	bool bShouldIgnoreOffsetAndAccuracy = false;
	...
	if (pPlayer && pWeapon)
	{
		...
		if (!pWeapon->m_bInReload && !bIsElites)
		{
			...
			float flCrouchAccuracy = pWeapon->GetInaccuracy();
			float flBaseAccuracy = pWeapon->GetInaccuracy();
...
float CalcVerticalBob(CBasePlayer* tfPlayer)
{
	static	float bobtime;
	static	float lastbobtime;
	float	cycle;

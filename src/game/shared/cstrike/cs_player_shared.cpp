...
const char* CCSPlayer::GetFootstepSound(const char* stepType, bool /*unused*/, float side, bool /*unused*/) const
{
	static char s_footstepSound[64];

	int team = GetTeamNumber();
	side = (random->RandomInt(1,2) == 2) ? 1.0f : 0.0f;

	// Special infected team
	if (team == 3)
	{
		const char* foot = (side > 0.0f) ? "StepRight" : "StepLeft";
		int zombieClass = GetZombieClass();
		if (GetWaterLevel() > 0) {
			// Infected have unique water footsteps
			// (except for Hunters, who use the same footstep sound in water and on land)

			switch (zombieClass)
			{
			case 3: // Hunter
				return "Hunter.Footstep";

			case 8: // Tank
				V_snprintf(s_footstepSound, sizeof(s_footstepSound),
					"Tank.Water.%s", foot);
				return s_footstepSound;
			...
			case 6: // Charger
			{
				foot = (side > 0.0f) ? "StepRight" : "StepLeft";
				V_snprintf(s_footstepSound, sizeof(s_footstepSound),
					"Charger.Default.%s", foot);
				return s_footstepSound;
			}
			}
...
void CCSPlayer::PlayStepSound(Vector& vecOrigin, surfacedata_t* psurface, float fvol, bool force)
{
	...
	else
	{
		...
		pSoundName = GetFootstepSound(pSoundName, !nSide, 0.0f, false);

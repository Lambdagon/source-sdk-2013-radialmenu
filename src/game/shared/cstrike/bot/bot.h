...
{
	// Hunter lunge becomes a pounce when the player is holding duck.
	// Ensure hunter bots always crouch for this command so their pounces are consistent.
	if ( this->GetZombieClass() == 3 )
	{
...
//--------------------------------------------------------------------------------------------------------------
template < class PlayerType >
inline void CBot< PlayerType >::Attack3( void )
{
	SETBITS( m_buttonFlags, IN_ATTACK3 );
}

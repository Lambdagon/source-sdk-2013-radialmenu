...
void CPredictedViewModel::ApplyViewModelPitchAndDip(CBasePlayer* owner, Vector& vecNewOrigin, QAngle& vecNewAngles)
{
	...
	float flOldFallVel = owner->m_Local.m_flFallVelocity;
...
void CPredictedViewModel::CalcViewModelView(const Vector& eyePosition, const QAngle& eyeAngles)
{
#if defined( CLIENT_DLL )
	C_BasePlayer *owner = C_BasePlayer::GetLocalPlayer();
	if (cl_use_new_headbob.GetBool() == false)

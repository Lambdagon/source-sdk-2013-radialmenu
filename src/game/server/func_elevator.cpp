...
{
	m_soundStart = NULL_STRING;
	m_soundStop = NULL_STRING;
	m_soundDisable = NULL_STRING;
	m_currentSound = NULL_STRING;
	m_flBlockDamage = 0.0f;

	m_topFloorPosition = vec3_origin;
	m_bottomFloorPosition = vec3_origin;
...
{
	const Vector vecSpawnOrigin = GetAbsOrigin();

	SetMoveType( MOVETYPE_CUSTOM );
	SetModel( STRING( GetModelName() ) );

	// Brush movers need their spawn position re-applied after model setup so the
	// elevator actually starts at its map-defined origin.
	UTIL_SetOrigin( this, vecSpawnOrigin );
	...
	while ( inputSource )
	{
		...
		while ( dmap && !found )
		{
			...
			for ( int i = 0; i < fields; i++ )
			{
				...
				{
					CBaseEntityOutput *pOutput = (CBaseEntityOutput *)((intp)inputSource + (int)dataDesc->fieldOffset[0]);

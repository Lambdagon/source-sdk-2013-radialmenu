...
private:
	...
	CountdownTimer m_rubberbandCooldown;						///< survivor squad rubberband cooldown
	Vector m_lastSquadGoal;										///< last computed formation goal (survivor squads)
...
public:
	...
	// Survivor squad system: allow urgent actions (rescue/revive) to ignore the squad leader follow.
	bool ShouldIgnoreSurvivorSquad( void ) const { return m_bIgnoreSurvivorSquad; }
	void SetIgnoreSurvivorSquad( bool ignore ) { m_bIgnoreSurvivorSquad = ignore; }
...
private:
	...
	mutable CHandle< CBaseEntity > m_commonInfectedAttacker;			///< last common infected that hurt us
	float m_commonInfectedAttackedTimestamp;						///< when we were hurt by m_commonInfectedAttacker
	float m_commonInfectedGrenadeTimestamp;						///< last time we started a grenade throw at common infected
	mutable CHandle< CBaseEntity > m_commonInfectedTarget;			///< current common infected we want to shoot/shove
	...
	Vector m_preStuckSpot;											///< the previous position before we became stuck (used for teleport recovery)

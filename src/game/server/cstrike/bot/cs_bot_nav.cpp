...
static Vector GetStuckRecoveryAnchor( CCSBot *me, const Vector &fallback )
{
	if ( !me )
		return fallback;

	CBaseEntity *enemy = me->GetBotEnemy();
	if ( enemy && enemy->IsAlive() )
		return enemy->GetAbsOrigin();

	if ( me->IsFollowing() && me->GetFollowLeader() )
		return me->GetFollowLeader()->GetAbsOrigin();
	...
	*/
	...
	if ( IsAttacking() )
	{
		...
		if ( IsUsingKnife() && m_enemy != NULL )
		{
			...
			if ( breakable )
			{
				...
				if ( IsSpecialInfected() && GetZombieClass() != 8 )
				{
					SecondaryAttack();
				}
				else
	...
	if ( IsLookingAtSpot( PRIORITY_HIGH ) && m_lookAtSpotAttack )
	{
		...
		else if ( GetActiveWeapon() )
		{
			...
			const bool canAttackNow = ( IsSpecialInfected() && GetZombieClass() != 8 ) ?
				( active->m_flNextSecondaryAttack <= gpGlobals->curtime ) :
				( active->m_flNextPrimaryAttack <= gpGlobals->curtime );
...
void CCSBot::DoorCheck( void )
{
	...
	if ( door )
	{
		// Special infected keep a high-priority "hunt" look-at almost constantly, which can
		// prevent DoorCheck() from ever engaging. Allow them to interrupt look-at to open doors.
		if ( IsSpecialInfected() || !IsLookingAtSpot( PRIORITY_HIGH ) )
		{
			if ( !IsOpeningDoor() )
			{
				OpenDoor( door );
...
{
	// Don't try to anti-stick (teleport/wiggle) while we're immobilized by gameplay (pounced/incapacitated).
	// These states intentionally prevent movement and can falsely trigger stuck recovery.
	if ( HasPounceAttacker() || HasPounceVictim() || IsIncapacitated() )
	{
		ResetStuckMonitor();
	...
	// If we're embedded in the world (or other solid), immediately apply the same anti-stick recovery
	// used for path wedging: teleport to a nearby nav area.
	{
		...
		trace_t solidTr;
		UTIL_TraceHull( curPos, curPos, mins, maxs, MASK_PLAYERSOLID, this, COLLISION_GROUP_PLAYER_MOVEMENT, &solidTr );

		if ( solidTr.startsolid || solidTr.allsolid )
		{
			const Vector referencePos = ( m_preStuckSpot != vec3_origin ) ? m_preStuckSpot : ( ( m_lastOrigin != vec3_origin ) ? m_lastOrigin : curPos );
			const Vector anchor = GetStuckRecoveryAnchor( this, referencePos );
			const float recoverRadius = 500.0f;

			if ( TeleportToRandomNavAreaNear( this, anchor, recoverRadius ) )
			{
				ResetStuckMonitor();
				m_lastOrigin = GetAbsOrigin();
				return;
	...
	if (m_isStuck)
	{
		...
		{
			// If we're stuck while trying to follow a path, teleport back onto the nav mesh near our previous position.
			// This avoids cases where wiggle/jump can't resolve getting wedged.
			const float stuckTeleportDelay = 1.0f;
			if ( m_stuckTimestamp > 0.0f &&
				 ( gpGlobals->curtime - m_stuckTimestamp ) >= stuckTeleportDelay &&
				 m_pathLength > 0 &&
				 TheNavMesh && TheNavMesh->IsLoaded() && !TheNavMesh->IsGenerating() )
			{
				const Vector referencePos = ( m_preStuckSpot != vec3_origin ) ? m_preStuckSpot : m_stuckSpot;
				const Vector anchor = GetStuckRecoveryAnchor( this, referencePos );
				const float recoverRadius = 500.0f;

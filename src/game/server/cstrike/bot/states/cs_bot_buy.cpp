...
void BuyState::OnEnter( CCSBot *me )
{
	...
	else
	{
		...
		{
			// Survivor bots should always attempt to buy on spawn.
			m_doneBuying = false;
		}
		else if (me->m_iAccount < cv_bot_eco_limit.GetFloat())
		{
	...
	// Survivor bots should buy immediately upon spawning.
	m_isInitialDelay = ( me->GetTeamNumber() == TEAM_SURVIVOR ) ? false : true;
...
void BuyState::OnUpdate( CCSBot *me )
{
	...
	if ( !TheNavMesh->IsLoaded() && me->GetTeamNumber() != TEAM_SURVIVOR )
		return;
	...
	if (!me->IsInBuyZone())
	{
		// Survivor bots can spawn on maps with no buy zones; allow them to buy anyway.
		if ( me->GetTeamNumber() != TEAM_SURVIVOR )
		{
			m_doneBuying = true;

...
void MoveToState::OnUpdate( CCSBot *me )
{
	...
	// Survivor bots: if our goal entity is an incapacitated survivor, move in and hold "use" to revive them.
	// Keep this at the top so it isn't overridden by scenario-specific MoveTo behavior (bombsites/hostages/etc).
	{
		...
		if ( reviveTarget && reviveTarget->GetTeamNumber() == TEAM_SURVIVOR )
		{
			...
			// If our target is far from our stored goal (should be rare), repath.
			...
			if ( !me->IsUsingLadder() )
			{
				const Vector revivePos = reviveTarget->EyePosition();
				const Vector to = revivePos - me->EyePosition(); // "use" checks from eye position

				const float watchRange = 250.0f;
				if ( to.IsLengthLessThan( watchRange ) )
				{
					me->SetLookAt( "Revive", revivePos, PRIORITY_UNINTERRUPTABLE, 0.25f );

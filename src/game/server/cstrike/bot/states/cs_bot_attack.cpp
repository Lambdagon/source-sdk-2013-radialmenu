...
void AttackState::OnUpdate( CCSBot *me )
{
	...
	// Hunter special infected: if we recently got hurt, disengage by lunging away and hiding.
	if ( me->IsSpecialInfected() && me->GetZombieClass() == 3 )
	{
		...
		if ( me->GetTimeSinceAttacked() < hurtWindow )
		{
			Vector away = myOrigin - enemyOrigin;
			away.z = 0.0f;
			...
			if ( !me->TryToHide( NULL, hunterHideDuration, hunterHideRange, true, USE_NEAREST ) )
			{
				me->DestroyPath();
				me->MoveAwayFromPosition( enemyOrigin );
			}
	...
	// Survivor bots: when engaging a Tank (hulk), kite it by running while continuously firing.
	// This keeps pressure on the Tank without letting the bot get cornered standing still.
	CCSPlayer *csEnemy = ToCSPlayer( enemy );
	if ( me->GetTeamNumber() == TEAM_SURVIVOR &&
		csEnemy &&
		csEnemy->GetTeamNumber() == TEAM_INFECTED &&
		csEnemy->GetZombieClass() == 8 &&
		me->IsEnemyVisible() )
	{
		m_crouchAndHold = false;
		me->StandUp();

			...
			IsEnemyVisible())
		{
			...
			if (onTarget > aimTolerance)
			{
				...
				if (doAttack)
				{
					...
					if (IsUsingKnife())
					{
						// Special infected melee uses secondary attack (primary reserved for abilities), except tank.
						// Boomer primary is vomit.
						// Boomer: try to vomit at mid-range; otherwise melee.
						if (GetZombieClass() == 2)
						{
							CBoomerClaw* claw = dynamic_cast<CBoomerClaw*>(GetActiveWeapon());
							if ( claw )
							{
								// While vomiting, don't melee yet.
								if ( claw->IsVomiting() )
								{
									return;
								}

								bool startedVomit = false;
								if ( enemy && claw->CanStartVomit() )
								{
									ConVarRef z_vomit_target_range("z_vomit_target_range");
									ConVarRef z_vomit_target_dot("z_vomit_target_dot");
									const float maxRange = z_vomit_target_range.IsValid() ? z_vomit_target_range.GetFloat() : 280.0f;
									const float minDot = z_vomit_target_dot.IsValid() ? z_vomit_target_dot.GetFloat() : 0.6f;

									const Vector myEye = EyePosition();
									Vector toEnemy = GetCentroid(enemy) - myEye;
									toEnemy.z = 0.0f;
									const float dist = toEnemy.Length();

									if ( dist <= maxRange && IsEnemyVisible() )
									{
										bool shouldVomit = false;

										// When we're very close, don't require perfect aim.
										const float closeVomitRange = 125.0f;
										if ( dist <= closeVomitRange )
										{
											shouldVomit = true;
										}
										else
								...
								// Vomit always comes before melee.
								if ( startedVomit )
								{
									return;
								}
						...
						{
							CSpitterClaw *claw = dynamic_cast< CSpitterClaw * >( GetActiveWeapon() );
							if ( claw && enemy && claw->CanStartSpit() )
							{
								ConVarRef z_spit_target_range( "z_spit_target_range" );
								const float maxRange = z_spit_target_range.IsValid() ? z_spit_target_range.GetFloat() : 500.0f;
						...
						{
							CHunterClaw *claw = dynamic_cast< CHunterClaw * >( GetActiveWeapon() );
							if ( claw && claw->IsLunging() )
							{
								// Let the lunge/pounce resolve before attempting melee.
								return;
							}
							...
							// If we haven't been hurt recently, prefer pouncing over running into melee range.
							if ( GetTimeSinceAttacked() >= hurtWindow && enemy && IsEnemyVisible() && GetGroundEntity() != NULL )
							{
								ConVarRef z_hunter_lunge_distance( "z_hunter_lunge_distance" );
								const float maxPounceRange = z_hunter_lunge_distance.IsValid() ? z_hunter_lunge_distance.GetFloat() : 750.0f;
								const float minPounceRange = 150.0f;

								if ( rangeToEnemy >= minPounceRange && rangeToEnemy <= maxPounceRange )
								{
									if ( claw )
									{
										claw->SetBotLungeTarget( enemy );
									}

									// Bot hunters need to look up a bit so their pounce trajectory has enough arc.
									ConVarRef z_hunter_lunge_pitch( "z_hunter_lunge_pitch" );
									const float maxExtraPitch = z_hunter_lunge_pitch.IsValid() ? z_hunter_lunge_pitch.GetFloat() : 25.0f;
									const float t = ( maxPounceRange > minPounceRange ) ?
										clamp( ( rangeToEnemy - minPounceRange ) / ( maxPounceRange - minPounceRange ), 0.0f, 1.0f ) :
										1.0f;
									const float extraPitch = MAX( 0.0f, maxExtraPitch ) * t;

									QAngle aimAngles;
									Vector toEnemy = GetCentroid( enemy ) - EyePosition();
									VectorAngles( toEnemy, aimAngles );
									aimAngles.x = clamp( aimAngles.x - extraPitch, -89.0f, 89.0f );
									aimAngles.z = 0.0f;
						...
						{
							// Hunter bots should not use the claw (melee) attack. Back up to set up a pounce instead.
							if ( IsSpecialInfected() && GetZombieClass() == 3 )
							{
								StandUp();
								Run();
								if ( enemy )
								{
									MoveAwayFromPosition( GetCentroid( enemy ) );
								}
								return;
							}
...
bool CCSBot::DoesActiveWeaponHaveSilencer( void ) const
{
	...
	// none of our weapons actually have a silencer, except for the silenced SMG
...
void CCSBot::EquipBestWeapon( bool mustEquip )
{
	...
	if (ctrl->AllowPistols() && GetTeamNumber() != 3)
	{
...
void CCSBot::EquipKnife( void )
{
	if (!IsUsingKnife())
	{
		...
		SelectItem("weapon_boomer_claw");
		SelectItem("weapon_smoker_claw");
		SelectItem("weapon_hunter_claw");
		SelectItem("weapon_tank_claw");
		SelectItem("weapon_charger_claw");
		SelectItem("weapon_jockey_claw");
		SelectItem("weapon_spitter_claw");
	}
}
...
bool CCSBot::IsUsingKnife( void ) const
{
	...
	if ( weapon )
	{
		if ( weapon->IsA( WEAPON_KNIFE ) )
			return true;

		// Special infected use various *_claw weapons that should be treated as melee/knife
		// for navigation behaviors (doors, ladders, etc).

...
public:
	...
	// "Special infected" bots (server-side gameplay feature).
	bool IsSpecialInfected( void ) const { return m_bIsSpecialInfected; }
	void SetSpecialInfected( bool isSpecialInfected );
...
public:
	...
	float m_nextPainSoundTime;
	float m_painSoundDuration;
	float m_nextZombieSoundTime;
	float m_nextVocalizeTime;
	float m_nextVocalizeFallTime;
	float m_vocalizeCooldown;
	bool m_bBoomerFallVocalized;
	float m_flNextWorldAntiStuckTime;
	float m_flNextRecognizeSoundTime;
	bool m_bWasSeeingEnemyLastUpdate;
	int m_nLastWarnedSpecialType;
	...
	// Survivor incapacitation (L4D-style).
	bool IsIncapacitated() const { return m_bIncapacitated; }
	int GetIncapacitationCount() const { return m_nIncapacitationCount; }
	bool HasIncapBlackAndWhite() const { return m_bIncapBlackAndWhite; }

	CCSPlayer *GetPounceVictim() const { return m_pounceVictim.Get(); }
	CCSPlayer *GetPounceAttacker() const { return m_pounceAttacker.Get(); }
	bool HasPounceVictim() const { return m_pounceVictim.Get() != NULL; }
	bool HasPounceAttacker() const { return m_pounceAttacker.Get() != NULL; }
...
private:
	...
	CountdownTimer m_zombieSoundTimer;
...
private:
	...
	CSoundPatch *m_pSpitterBurnMusic;
	float m_flLastSpitterAcidDamageTime;
...
private:
	...
	bool			m_bIsSpecialInfected;
	float			m_flSpecialInfectedDeathTimestamp;

	CTakeDamageInfo	m_TankDeathInfo;

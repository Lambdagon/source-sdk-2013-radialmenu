...
ConVar z_hunter_health( "z_hunter_health", "250", FCVAR_GAMEDLL | FCVAR_CHEAT, "Zombie max health" );
ConVar z_hunter_limit( "z_hunter_limit", "1", FCVAR_GAMEDLL | FCVAR_CHEAT, "" );
ConVar z_hunter_speed( "z_hunter_speed", "300", FCVAR_GAMEDLL, "" );
...
{
	GHOST_SPAWN_FEEDBACK_NONE = 0,
	GHOST_SPAWN_FEEDBACK_BLOCKED,
	GHOST_SPAWN_FEEDBACK_READY,
};

static bool IsGhostSpawnHiddenFromSurvivorLOS( CCSPlayer *survivor, const Vector &spawnPos )
{
	if ( !survivor )
		return false;
...
static bool IsGhostSpawnHiddenFromAllSurvivorsLOS( const Vector &spawnPos )
{
	for ( int i = 1; i <= gpGlobals->maxClients; ++i )
	{
		CCSPlayer *survivor = ToCSPlayer( UTIL_PlayerByIndex( i ) );
		if ( !survivor || !survivor->IsAlive() || survivor->GetTeamNumber() != TEAM_SURVIVOR )
			continue;

		if ( !IsGhostSpawnHiddenFromSurvivorLOS( survivor, spawnPos ) )
			return false;
	}
...
static QAngle GetChargerVictimFacingAngles( const CCSPlayer *pCharger )
{
	...
	return QAngle( 0.0f, pCharger->GetAbsAngles().y, 0.0f );
}

static void KeepChargerVictimAttached( CCSPlayer *pCharger, CCSPlayer *pVictim, const Vector &carryPos )
{
	if ( !pCharger || !pVictim )
		return;

	const QAngle victimAngles = GetChargerVictimFacingAngles( pCharger );
	Vector victimVelocity( vec3_origin );

	pVictim->Teleport( &carryPos, &victimAngles, &victimVelocity );
	pVictim->SnapEyeAngles( victimAngles );
	pVictim->SetAbsVelocity( vec3_origin );
	pVictim->SetBaseVelocity( vec3_origin );

	PhysDisableEntityCollisions( pCharger, pVictim );
}

static void CollectChargerChargeVictimsAlongSweep(
	CCSPlayer *pCharger,
	const Vector &start,
	const Vector &end,
	const Vector &mins,
	const Vector &maxs,
	CCSPlayer **ppPrimaryVictim,
	float *pPrimaryFraction,
	CUtlVector< CCSPlayer * > *pAdditionalVictims )
{
	if ( ppPrimaryVictim )
	{
		*ppPrimaryVictim = NULL;
	}

	if ( pPrimaryFraction )
	{
		*pPrimaryFraction = 1.0f;
	}

	if ( pAdditionalVictims )
	{
		pAdditionalVictims->Purge();
	}
	...
	CBaseEntity *entities[64];
	const int entityCount = UTIL_EntitiesAlongRay( entities, ARRAYSIZE( entities ), ray, FL_CLIENT );
	CCSPlayer *pCurrentVictim = pCharger->GetChargerVictim();

	auto AddUniqueVictim = [&]( CCSPlayer *pVictim )
	{
		if ( !pAdditionalVictims || !pVictim )
			return;

		for ( int i = 0; i < pAdditionalVictims->Count(); ++i )
		{
			if ( (*pAdditionalVictims)[i] == pVictim )
				return;
		}

		pAdditionalVictims->AddToTail( pVictim );
	};

	CCSPlayer *pBestVictim = NULL;
	float flBestFraction = 1.0f;

	for ( int i = 0; i < entityCount; ++i )
	{
		CCSPlayer *pVictim = ToCSPlayer( entities[i] );
		if ( !pVictim || pVictim == pCharger || pVictim == pCurrentVictim )
			continue;
		if ( !pVictim->IsAlive() || pVictim->GetTeamNumber() != TEAM_SURVIVOR )
			continue;

		trace_t playerTrace;
		enginetrace->ClipRayToEntity( ray, MASK_PLAYERSOLID | CONTENTS_HITBOX, pVictim, &playerTrace );
		if ( !playerTrace.startsolid && !playerTrace.allsolid && playerTrace.fraction >= 1.0f )
			continue;

		const float flFraction = ( playerTrace.startsolid || playerTrace.allsolid ) ? 0.0f : playerTrace.fraction;
		if ( !pBestVictim )
		{
			AddUniqueVictim( pBestVictim );
			pBestVictim = pVictim;
			flBestFraction = flFraction;
		}
		else
		{
			AddUniqueVictim( pVictim );
		}
	}

	if ( ppPrimaryVictim )
	{
		*ppPrimaryVictim = pBestVictim;
	}

	if ( pPrimaryFraction )
	{
		*pPrimaryFraction = flBestFraction;
	}
}
//=============================================================================
// HPE_BEGIN:
...
IMPLEMENT_SERVERCLASS_ST( CCSPlayer, DT_CSPlayer )
	...
	SendPropInt(SENDINFO(m_zombieClass), 8, SPROP_UNSIGNED),
	SendPropInt(SENDINFO(m_survivorClass), 8, SPROP_UNSIGNED),
	SendPropBool(SENDINFO(m_bTankDeathInProgress)),
	SendPropFloat(SENDINFO(m_flTankDeathPrevCycle), 0, SPROP_NOSCALE ),
	SendPropInt(SENDINFO(m_nTankDeathSequence), 8, SPROP_UNSIGNED),
	...
	SendPropBool( SENDINFO( m_bIsIT ) ),
	SendPropBool( SENDINFO( m_bIsGhost ) ),
	...
	SendPropInt( SENDINFO( m_nDamageStaggerDir ), 3, SPROP_UNSIGNED ),
...
static CSoundPatch *g_pTankMusic = NULL;
static int g_nTankMusicActiveTanks = 0;

static void TankMusic_Start()
{
	if ( g_pTankMusic )
		return;
	...
	g_pTankMusic = controller.SoundCreate( filter, SOUND_FROM_WORLD, CHAN_STATIC, "Event.Tank", ATTN_NONE );
	if ( g_pTankMusic )
	{
		controller.Play( g_pTankMusic, 1.0f, 100 );
	}
}

static void TankMusic_Stop( float flFadeTime )
{
	if ( !g_pTankMusic )
		return;

	CSoundEnvelopeController::GetController().SoundFadeOut( g_pTankMusic, flFadeTime, true );
	g_pTankMusic = NULL;
}

void TankMusic_AddTank()
{
	++g_nTankMusicActiveTanks;
	if ( g_nTankMusicActiveTanks == 1 )
	{
		TankMusic_Start();
	}
}

static void TankMusic_RemoveTank()
{
	if ( g_nTankMusicActiveTanks > 0 )
	{
		--g_nTankMusicActiveTanks;
	}
	...
	if ( g_nTankMusicActiveTanks <= 0 )
	{
		g_nTankMusicActiveTanks = 0;
		TankMusic_Stop( 2.0f );
	}
}

static bool TeleportPlayerToNearestNavArea( CCSPlayer *player, const Vector &anchor )
{
	if ( !player )
		return false;
	...
	CNavArea *area = TheNavMesh->GetNearestNavArea( anchor );
	if ( !area )
		return false;
...
CCSPlayer::CCSPlayer()
{
	...
	m_isVIP = false;
	m_bIsSpecialInfected = false;
	m_flSpecialInfectedDeathTimestamp = 0.0f;
	m_bIsGhost = false;

	m_bTankDeathInProgress = false;
	...
	m_flDamageStaggerPrevCycle = 0.0f;
	m_angDamageStaggerEyeAngles.Init();
	...
	m_vecChargerTracePos = vec3_origin;
	m_ChargerChargeHitVictims.Purge();
	m_bChargerChargeCapturedVictim = false;
	...
	m_bBoomerFallVocalized = false;
	m_flNextWorldAntiStuckTime = 0.0f;
	...
	m_pSpitterBurnMusic = NULL;
	m_flLastSpitterAcidDamageTime = 0.0f;
	m_bTankMusicCounted = false;
...
CCSPlayer::~CCSPlayer()
{
	...
	{
		TankMusic_RemoveTank();
		m_bTankMusicCounted = false;
	}

	if ( m_pSpitterBurnMusic )
	{
		CSoundEnvelopeController::GetController().SoundDestroy( m_pSpitterBurnMusic );
		m_pSpitterBurnMusic = NULL;
	}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CCSPlayer::ModifyOrAppendCriteria(AI_CriteriaSet& criteriaSet)
{
	BaseClass::ModifyOrAppendCriteria(criteriaSet);
	if (survivor_set.GetInt() == 1) {

		if (GetSurvivorClass() == 0) {
			criteriaSet.AppendCriteria("Who", "NamVet");
		}
		if (GetSurvivorClass() == 1) {
			criteriaSet.AppendCriteria("Who", "Biker");
		}
		if (GetSurvivorClass() == 2) {
			criteriaSet.AppendCriteria("Who", "Manager");
		}
		if (GetSurvivorClass() == 3) {
			criteriaSet.AppendCriteria("Who", "TeenGirl");
		}
		...
		if (GetSurvivorClass() == 0) {
			criteriaSet.AppendCriteria("Who", "Gambler");
		}
		if (GetSurvivorClass() == 1) {
			criteriaSet.AppendCriteria("Who", "Coach");
		}
		if (GetSurvivorClass() == 2) {
			criteriaSet.AppendCriteria("Who", "Mechanic");
		}
		if (GetSurvivorClass() == 3) {
			criteriaSet.AppendCriteria("Who", "Producer");
		}

	}
	criteriaSet.AppendCriteria("Coughing", "0");
	criteriaSet.AppendCriteria("Incapacitated", m_bIncapacitated ? "1" : "0");
}
...
{
	// Always transmit players so they can always be rendered client-side (no PVS filtering).
	return FL_EDICT_ALWAYS;
}

void CCSPlayer::SetSpecialInfected( bool isSpecialInfected )
{
	m_bIsSpecialInfected = isSpecialInfected;
	m_flSpecialInfectedDeathTimestamp = 0.0f;
}

void CCSPlayer::SetGhost( bool isGhost )
{
	if ( m_bIsGhost == isGhost )
		return;
	...
	if ( m_bIsGhost && !IsBot() && IsAlive() && GetTeamNumber() == TEAM_INFECTED && GetZombieClass() > 0 )
	{
		EmitPrivateSound( "PlayerZombie.BecomeGhost" );
	}

	CollisionRulesChanged();
}

bool CCSPlayer::CanExitGhost( const char **outMessage ) const
{
	if ( outMessage )
	{
		*outMessage = NULL;
	...
	if ( m_iHealth < 0 || GetTeamNumber() != TEAM_INFECTED || GetZombieClass() <= 0 )
		return false;

	const Vector origin = GetAbsOrigin();
	const float safetyRadiusSqr = kGhostSpawnSafetyRadius * kGhostSpawnSafetyRadius;
	...
	for ( int i = 1; i <= gpGlobals->maxClients; ++i )
	{
		...
		if ( survivor->GetAbsOrigin().DistToSqr( origin ) < safetyRadiusSqr )
		{
			if ( outMessage )
			{
				*outMessage = "Can't spawn here\nYou are too close to the Survivors";
			}
			return false;
		}
	}

	if ( !IsGhostSpawnHiddenFromAllSurvivorsLOS( origin ) )
	{
		if ( outMessage )
		{
			*outMessage = "Can't spawn here\nYou can be seen by the Survivors";
		}
		return false;
	}

	const Vector mins = m_Local.m_bDucked ? VEC_DUCK_HULL_MIN_SCALED( this ) : VEC_HULL_MIN_SCALED( this );
	const Vector maxs = m_Local.m_bDucked ? VEC_DUCK_HULL_MAX_SCALED( this ) : VEC_HULL_MAX_SCALED( this );

	trace_t tr;
	UTIL_TraceHull( origin, origin, mins, maxs, MASK_PLAYERSOLID, const_cast< CCSPlayer * >( this ), COLLISION_GROUP_NONE, &tr );
	if ( tr.startsolid || tr.allsolid )
	{
		if ( outMessage )
		{
			*outMessage = "Can't spawn here\nSomething is blocking this spot";
		}
		return false;
	}
	...
	if ( outMessage )
	{
		*outMessage = "Press RIGHT MOUSE BUTTON to spawn.";
	}

	return true;
}

bool CCSPlayer::UpdateGhostSpawnFeedback( bool bPrintMessage, bool bPrintReadyMessage )
{
	const char *pszGhostMessage = NULL;
	const bool bCanSpawn = CanExitGhost( &pszGhostMessage );
	const int nGhostSpawnFeedbackState = bCanSpawn ? GHOST_SPAWN_FEEDBACK_READY : GHOST_SPAWN_FEEDBACK_BLOCKED;

	if ( !IsBot() && nGhostSpawnFeedbackState != m_nGhostSpawnFeedbackState )
	{
		EmitPrivateSound( bCanSpawn ? "PZ.Hud.Action" : "PZ.Hud.BadPlace" );
	}

	m_nGhostSpawnFeedbackState = nGhostSpawnFeedbackState;

	if ( bPrintMessage && pszGhostMessage && ( !bCanSpawn || bPrintReadyMessage ) )
	{
		ClientPrint( this, HUD_PRINTCENTER, pszGhostMessage );
	}

	return bCanSpawn;
}

void CCSPlayer::KickSpecialInfectedIfDeadLongEnough( float deadSeconds )
{
	if ( !m_bIsSpecialInfected || !IsBot() )
		return;
	...
	if ( m_flSpecialInfectedDeathTimestamp <= 0.0f )
		return;

	if ( gpGlobals->curtime < ( m_flSpecialInfectedDeathTimestamp + deadSeconds ) )
		return;
}
...
void CCSPlayer::Precache()
{
	...
	PrecacheModel("models/weapons/arms/v_arms_coach_new.mdl");
	PrecacheModel("models/weapons/arms/v_arms_mechanic_new.mdl");
	PrecacheModel("models/weapons/arms/v_arms_producer_new.mdl");
	PrecacheModel("models/weapons/arms/v_arms_bill.mdl");
	PrecacheModel("models/weapons/arms/v_arms_francis.mdl");
	PrecacheModel("models/weapons/arms/v_arms_louis.mdl");
	PrecacheModel("models/weapons/arms/v_arms_zoey.mdl");
	PrecacheModel("models/empty.mdl");
	PrecacheParticleSystem("charger_wall_impact");
	...
	PrecacheModel("models/infected/limbs/exploded_boomer_head.mdl");
	PrecacheModel("models/infected/limbs/exploded_boomer_rarm.mdl");
	...
	PrecacheModel("models/infected/limbs/exploded_boomer_steak3.mdl");
	...
	PrecacheScriptSound("Event.MissionStart_NoMercy");
	PrecacheScriptSound("Event.LeavingSafety_NoMercy");
	PrecacheScriptSound("Event.BackgroundMap");
	PrecacheScriptSound( "Event.Tank" );
	PrecacheScriptSound("Zombie.Punch");
	PrecacheScriptSound("PlayerZombie.AttackHit");
	PrecacheScriptSound("ChargerZombie.Stagger");
	PrecacheScriptSound("ChargerZombie.HitPerson");
	PrecacheScriptSound("ChargerZombie.Alert");
	PrecacheScriptSound("ChargerZombie.Warn");
	PrecacheScriptSound("ChargerZombie.Voice");
	PrecacheScriptSound("ChargerZombie.Pain");
	PrecacheScriptSound("ChargerZombie.Death");
	PrecacheScriptSound("ChargerZombie.Recognize");
	PrecacheScriptSound("ChargerZombie.Charge");
	PrecacheScriptSound("ChargerZombie.ChargeEnd");
	PrecacheScriptSound("ChargerZombie.Melee");
	PrecacheScriptSound("ChargerZombie.Pummel");
	PrecacheScriptSound("ChargerZombie.VocalizePummel");
	PrecacheScriptSound("ChargerZombie.ImpactHard");
	...
	PrecacheScriptSound("Player.JumpLand");
	PrecacheScriptSound("PlayerZombie.JumpLand");
	PrecacheScriptSound("Player.HitInternal");
	PrecacheScriptSound("Player.Heartbeat");
	PrecacheScriptSound("Player.Fall");
	PrecacheScriptSound("Player.BandagingWounds");
	PrecacheScriptSound("Player.AwardUpgrade");
	PrecacheScriptSound("Player.NeckSnap");
	PrecacheScriptSound("Blood.Splat");
	PrecacheScriptSound("Vomit.Use");
	PrecacheScriptSound("Player.UsingGasCan");
	PrecacheScriptSound("Player.UsingGasCanStop");
	PrecacheScriptSound("SmokerZombie.Pain");
	PrecacheScriptSound("SmokerZombie.PainShort");
	PrecacheScriptSound("SmokerZombie.Alert");
	PrecacheScriptSound("SmokerZombie.Warn");
	PrecacheScriptSound("SmokerZombie.TongueAttack");
	PrecacheScriptSound("SmokerZombie.TongueHit");
	PrecacheScriptSound("SmokerZombie.TongueRetract");
	PrecacheScriptSound("SmokerZombie.TongueFly");
	PrecacheScriptSound("Player.Choke");
	PrecacheScriptSound("HulkZombie.Thud");
	PrecacheScriptSound("HulkZombie.Pain");
	PrecacheScriptSound("HulkZombie.PainFire");
	PrecacheScriptSound("PlayerZombie.BecomeGhost");
	PrecacheScriptSound("PZ.Hud.BadPlace");
	PrecacheScriptSound("PZ.Hud.Action");
	PrecacheScriptSound("PlayerZombie.BecomeReal");
	PrecacheScriptSound("PlayerZombie.Rage");
	PrecacheScriptSound("PlayerZombie.Breathe");
	PrecacheScriptSound("PlayerZombie.Attack");
	PrecacheScriptSound("PlayerZombie.AttackMiss");
	PrecacheScriptSound("PlayerZombie.AttackHit");
	PrecacheScriptSound("PlayerZombie.Hurt");
	PrecacheScriptSound("PlayerZombie.Die");
	PrecacheScriptSound("PlayerZombie.CullWarn");
	PrecacheScriptSound("PlayerZombie.Fall");
	PrecacheScriptSound("PlayerZombie.Thud");
	PrecacheScriptSound("HunterZombie.LungeLand");
	PrecacheScriptSound("HunterZombie.Pounce");
	PrecacheScriptSound("HunterZombie.Lunge");
	PrecacheScriptSound("HunterZombie.Pounce.Cancel");
	PrecacheScriptSound("HunterZombie.Pounce.Hit");
	PrecacheScriptSound("HunterZombie.Pounce.FlyLoop");
	PrecacheScriptSound("HunterZombie.Pounce.Miss");
	PrecacheScriptSound("HunterZombie.Growl");
	PrecacheScriptSound("HunterZombie.Warn");
	PrecacheScriptSound("HunterZombie.Alert");
	PrecacheScriptSound("HunterZombie.Pounce.shred");
	PrecacheScriptSound("HunterZombie.Death");
	PrecacheScriptSound("HunterZombie.Pain");
	PrecacheScriptSound("HunterZombie.PainShort");
	PrecacheScriptSound("BoomerZombie.Alert");
	PrecacheScriptSound("BoomerZombie.Pain");
	PrecacheScriptSound("BoomerZombie.Voice");
	PrecacheScriptSound("BoomerZombie.Groan");
	PrecacheScriptSound("BoomerZombie.PainShort");
	PrecacheScriptSound("BoomerZombie.Attack");
	PrecacheScriptSound("BoomerZombie.Fall");
	PrecacheScriptSound("BoomerZombie.Detonate");
	PrecacheScriptSound("JockeyZombie.Ride");
	PrecacheScriptSound("Hint.Critical");
	PrecacheScriptSound("Hint.Helpful");
	PrecacheScriptSound("Hint.LittleReward");
	PrecacheScriptSound("Hint.BigReward");
	PrecacheScriptSound("Instructor.LessonStart");
	PrecacheScriptSound("Instructor.ImportantLessonStart");
	PrecacheScriptSound("Scavenge.point_scored");
	PrecacheScriptSound("Scavenge.zero_time");
	PrecacheScriptSound("ScavengeSB.ZeroScore");
	PrecacheScriptSound("ScavengeSB.OpenTiePanel");
	PrecacheScriptSound("ScavengeSB.DisplayChange");
	PrecacheScriptSound("ScavengeSB.RoundScoreIncrement");
	PrecacheScriptSound("ScavengeSB.MatchScoreIncrement");
	PrecacheScriptSound("ScavengeSB.RoundTimeIncrement");
	PrecacheScriptSound("ScavengeSB.MatchScoreFinal");
	PrecacheScriptSound("Scavenge.ClusterSpawned");
	PrecacheScriptSound("Weapon.Swing");
	PrecacheScriptSound("Weapon.HitSurvivor");
	PrecacheScriptSound("Weapon.HitWorld");
	PrecacheScriptSound("Weapon.HitBoomer");
	PrecacheScriptSound("Weapon.HitInfected");
	PrecacheScriptSound("Weapon.HitClown");
	PrecacheScriptSound("Knife.StabHunter");
	PrecacheScriptSound("Knife.BreakTongue");
	PrecacheScriptSound("BlindingFlash");
	PrecacheScriptSound("ChargerZombie.Smash");
	PrecacheScriptSound("Player.LaserOn");
	PrecacheScriptSound("Event.NewAvailableZombie");
	PrecacheScriptSound("HunterZombie.LungeLand");
	PrecacheScriptSound("HunterZombie.Pounce");
	PrecacheScriptSound("HunterZombie.Lunge");
	PrecacheScriptSound("HunterZombie.Pounce.Cancel");
	...
	PrecacheScriptSound("HunterZombie.Alert");
	PrecacheScriptSound("HunterZombie.Pain");
	PrecacheScriptSound("JockeyZombie.Growl");
	PrecacheScriptSound("JockeyZombie.Warn");
	PrecacheScriptSound("JockeyZombie.Alert");
	PrecacheScriptSound("JockeyZombie.Pain");
	PrecacheScriptSound("JockeyZombie.PainShort");
	PrecacheScriptSound("JockeyZombie.Voice");
	PrecacheScriptSound("JockeyZombie.Ride");
	PrecacheScriptSound("JockeyZombie.Death");
	PrecacheScriptSound("JockeyZombie.Recognize");
	PrecacheScriptSound("Zombie.Punch");
	PrecacheScriptSound("SmokerZombie.Recognize");
	PrecacheScriptSound("SmokerZombie.Attack");
	PrecacheScriptSound("SmokerZombie.Breathe");
	PrecacheScriptSound("SmokerZombie.Death");
	PrecacheScriptSound("SmokerZombie.Explode");
	PrecacheScriptSound("SmokerZombie.Attack");
	PrecacheScriptSound("HulkZombie.Breathe");
	PrecacheScriptSound("HulkZombie.Growl");
	PrecacheScriptSound("HulkZombie.Yell");
	PrecacheScriptSound("HulkZombie.Attack");
	...
	PrecacheScriptSound("Blood.Spurt");
	PrecacheScriptSound("Melee.HitHead");
	PrecacheScriptSound("Melee.HitBody");
	PrecacheScriptSound("Melee.HitLimb");
	PrecacheScriptSound("CEDA.suit.deflate");
	PrecacheScriptSound("SpitterZombie.Voice");
	PrecacheScriptSound("SpitterZombie.Pain");
	PrecacheScriptSound("SpitterZombie.PainShort");
	PrecacheScriptSound("SpitterZombie.Recognize");
	PrecacheScriptSound("SpitterZombie.Attack");
	PrecacheScriptSound("SpitterZombie.Alert");
	PrecacheScriptSound("SpitterZombie.Death");
	PrecacheScriptSound("SpitterZombie.Spit");
...
void CCSPlayer::PlayerRunCommand( CUserCmd *ucmd, IMoveHelper *moveHelper )
{
	...
	if ( IsGhost() )
	{
		const bool bTrySpawn = ( ucmd->buttons & IN_ATTACK2 ) != 0;
		ucmd->buttons &= ~( IN_ATTACK | IN_ATTACK2 );

		if ( bTrySpawn && m_flGhostMaterializeTime <= 0.0f )
		{
			if ( UpdateGhostSpawnFeedback( true, false ) )
			{
				EmitSound("PlayerZombie.BecomeReal");
				m_flGhostMaterializeTime = gpGlobals->curtime + kGhostMaterializeDelay;
				m_nGhostSpawnFeedbackState = GHOST_SPAWN_FEEDBACK_NONE;
			}

			m_flNextGhostStatusMessageTime = gpGlobals->curtime + kGhostStatusMessageInterval;
		}
	}

	// Incapacitated survivors can't move and are forced to crouch; they can only use their pistol.
	if ( m_bIncapacitated )
	{
		ucmd->forwardmove = ucmd->sidemove = ucmd->upmove = 0.0f;
		ucmd->buttons |= IN_DUCK;
	...
	if ( HasPounceAttacker() || HasPounceVictim() )
	{
		...
		// Lock server-side eye angles while pounced so players can't rotate their character,
		// but avoid FIXANGLE (which would also lock the client camera).
		if ( HasPounceVictim() )
		{
			CCSPlayer *pVictim = GetPounceVictim();
			if ( pVictim )
			{
				QAngle lockedAngles;
				VectorAngles( pVictim->WorldSpaceCenter() - WorldSpaceCenter(), lockedAngles );
				lockedAngles.x = 0.0f;
				lockedAngles.z = 0.0f;
		...
		{
			CCSPlayer *pAttacker = GetPounceAttacker();
			if ( pAttacker )
			{
				QAngle lockedAngles;
				VectorAngles( pAttacker->WorldSpaceCenter() - WorldSpaceCenter(), lockedAngles );
				lockedAngles.x = 0.0f;
				lockedAngles.z = 0.0f;
				ucmd->viewangles = lockedAngles;
			}
	...
	if ( HasChargerAttacker() )
	{
		...
		CCSPlayer *pAttacker = GetChargerAttacker();
		if ( pAttacker && pAttacker->GetTeamNumber() == TEAM_INFECTED && pAttacker->GetZombieClass() == 6 )
		{
			QAngle lockedAngles( 0.0f, pAttacker->GetAbsAngles().y, 0.0f );
			ucmd->viewangles = lockedAngles;
		}
	}
	else if ( GetTeamNumber() == TEAM_INFECTED && GetZombieClass() == 6 )
	{
		...
		{
			// Don't allow steering/movement inputs while charging. We drive velocity explicitly in PostThink.
			ucmd->forwardmove = ucmd->sidemove = ucmd->upmove = 0.0f;
			ucmd->buttons &= ~( IN_ATTACK | IN_ATTACK2 | IN_JUMP );

			// Lock server-side eye angles so the charger can't steer, but don't use FIXANGLE (which would lock the client camera).
		...
		{
			// Pounding: fully locked, and keep server-side eye angles fixed like hunter pounce.
			ucmd->forwardmove = ucmd->sidemove = ucmd->upmove = 0.0f;
			ucmd->buttons &= ~( IN_ATTACK | IN_ATTACK2 | IN_JUMP );
	...
	else if ( m_nDamageStaggerDir != PLAYER_STAGGER_DIR_NONE )
	{
		ucmd->forwardmove = ucmd->sidemove = ucmd->upmove = 0.0f;
		ucmd->buttons &= ~( IN_ATTACK | IN_ATTACK2 | IN_JUMP );
	}
...
static CCSPlayer *SelectRandomAliveSurvivorT()
{
	CCSPlayer *choice = NULL;
	int count = 0;
...
void CCSPlayer::Spawn()
{
	...
	// Reset any pending post-death cleanup state.
	m_flSpecialInfectedDeathTimestamp = 0.0f;
	SetGhost( false );
	m_bTankDeathInProgress = false;
	m_flTankDeathPrevCycle = 0.0f;
	...
	if (GetTeamNumber() == TEAM_INFECTED) {
		GetViewModel(1)->SetModel("models/empty.mdl");
	}
	//=============================================================================
	// HPE_BEGIN:
	...
	StockPlayerAmmo();
	if (GetTeamNumber() == TEAM_INFECTED) {
		const bool bSpawnAsGhost = !IsBot();

		SetSurvivorClass(0);

		// By default, infected players spawn as a random special infected.
		// Special infected bots can optionally force a class (e.g. tank) before spawning.
		if ( !IsSpecialInfected() || GetZombieClass() <= 0 )
		{
			SetZombieClass( random->RandomInt( 1, 6 ) );
		}
		if (GetZombieClass() == 1) {
			if (survivor_set.GetInt() == 1) {
				SetModel("models/infected/smoker_l4d1.mdl");
			}
			else {
				SetModel("models/infected/smoker.mdl");
			}
			m_iHealth = 250;
			SetMaxHealth(250);
			SetMaxSpeed(210);
			...
			if (survivor_set.GetInt() == 1) {
				SetModel("models/infected/boomer_l4d1.mdl");
			}
			else {
				if (random->RandomFloat(0.0f, 1.0f) < 0.25f)
				{
					SetModel("models/infected/boomette.mdl");
				}
				else
				{
					SetModel("models/infected/boomer.mdl");
				}
			}
			m_iHealth = 50;
			SetMaxHealth(50);
			...
			if (survivor_set.GetInt() == 1) {
				SetModel("models/infected/hunter_l4d1.mdl");
			}
			else {
				SetModel("models/infected/hunter.mdl");
			}
			...
			if (survivor_set.GetInt() == 1) {
				SetModel("models/infected/hulk_l4d1.mdl");
			}
			else {
		...
		RemoveAllWeapons();
		if (GetZombieClass() == 1) {
			GiveNamedItem("weapon_smoker_claw");
		}
		else if (GetZombieClass() == 2) {
			GiveNamedItem("weapon_boomer_claw");
		}
		else if (GetZombieClass() == 3) {
			GiveNamedItem("weapon_hunter_claw");
		}
		else if (GetZombieClass() == 4) {
			GiveNamedItem("weapon_spitter_claw");
		}
		else if (GetZombieClass() == 5) {
			GiveNamedItem("weapon_jockey_claw");
		}
		else if (GetZombieClass() == 6) {
			GiveNamedItem("weapon_charger_claw");
		}
		else if (GetZombieClass() == 8) {
			GiveNamedItem("weapon_tank_claw");
		}

		// Use the director spawn teleport system instead of static CT spawn points.
		if ( GetTeamNumber() == TEAM_INFECTED && CSGameRules() && !CSGameRules()->IsFreezePeriod() )
		{
			CCSPlayer *survivor = CSGameRules()->GetItTarget();
			if ( !survivor || !survivor->IsAlive() || survivor->GetTeamNumber() != TEAM_SURVIVOR )
			{
			...
			Vector spawnPos;
			QAngle spawnAng;
			if ( survivor && CSGameRules()->FindSpecialInfectedSpawnPos( survivor, &spawnPos, &spawnAng ) )
			{
				Vector vel( vec3_origin );
				Teleport( &spawnPos, &spawnAng, &vel );
			}
		}

		SetGhost( bSpawnAsGhost );
	}
		...
		SetSurvivorClass(survivorClass);
...
void CCSPlayer::CreateRagdollEntity()
{
	...
	if (IsOnFire()) {
		pRagdoll->Ignite(10.0f, false, 0.0f, false);
	}
	// ragdolls will be removed on round restart automatically
	m_hRagdoll = pRagdoll;
}

static Vector ComputeDamageShoveDir( CCSPlayer *pVictim, const CTakeDamageInfo &info )
{
	Vector dir = vec3_origin;

	if ( info.GetDamageType() & DMG_BLAST )
	{
		Vector source = vec3_origin;
		if ( info.GetDamagePosition() != vec3_origin )
		{
			source = info.GetDamagePosition();
		}
		else if ( info.GetInflictor() )
		{
			source = info.GetInflictor()->WorldSpaceCenter();
		}
		else if ( info.GetAttacker() )
		{
			source = info.GetAttacker()->WorldSpaceCenter();
		}

		if ( source != vec3_origin )
		{
			dir = pVictim->WorldSpaceCenter() - source;
		}
	...
	else
	{
		if ( info.GetAttacker() )
		{
			dir = pVictim->WorldSpaceCenter() - info.GetAttacker()->WorldSpaceCenter();
		}
		else if ( info.GetDamageForce() != vec3_origin )
		{
			dir = info.GetDamageForce();
	...
	dir.z = 0.0f;
	if ( dir.NormalizeInPlace() <= 0.0f )
		return vec3_origin;

	return dir;
}

static PlayerStaggerDir_t ComputePlayerStaggerDir( CCSPlayer *pVictim, const Vector &shoveDir, Activity *pActivity )
{
	Vector forward, right;
	pVictim->GetVectors( &forward, &right, NULL );
	forward.z = 0.0f;
	right.z = 0.0f;

	if ( forward.NormalizeInPlace() <= 0.0f || right.NormalizeInPlace() <= 0.0f )
		return PLAYER_STAGGER_DIR_NONE;

	const float forwardDot = DotProduct( forward, shoveDir );
	const float rightDot = DotProduct( right, shoveDir );

	PlayerStaggerDir_t dir = PLAYER_STAGGER_DIR_NONE;
	...
	if ( fabsf( forwardDot ) >= fabsf( rightDot ) )
	{
		if ( forwardDot >= 0.0f )
		{
			dir = PLAYER_STAGGER_DIR_FORWARD;
			activity = ACT_TERROR_SHOVED_FORWARD;
		}
		else
		{
			dir = PLAYER_STAGGER_DIR_BACK;
			activity = ACT_TERROR_SHOVED_BACKWARD;
		}
	}
	else if ( rightDot >= 0.0f )
	{
		dir = PLAYER_STAGGER_DIR_RIGHT;
		activity = ACT_TERROR_SHOVED_RIGHTWARD;
	}
	else
	{
		dir = PLAYER_STAGGER_DIR_LEFT;
		activity = ACT_TERROR_SHOVED_LEFTWARD;
	}
...
bool CCSPlayer::TryStartTankDeath( const CTakeDamageInfo &info )
{
	if ( !IsAlive() )
		return false;
	...
	const bool bWasMoving = ( GetAbsVelocity().Length2D() > 10.0f );
	const Activity deathAct = bWasMoving ? ACT_TERROR_DIE_WHILE_RUNNING : ACT_TERROR_DIE_FROM_STAND;
	const int seq = SelectWeightedSequence( deathAct );
	if ( seq == -1 )
		return false;

	m_bTankDeathInProgress = true;
	m_flTankDeathPrevCycle = 0.0f;
	m_nTankDeathSequence = seq;
	m_TankDeathInfo = info;

	// Keep the player "alive" so the client draws the model, but stop gameplay interaction.
	m_iHealth = 1;
	m_takedamage = DAMAGE_NO;
	...
	EmitSound( "HulkZombie.Die" );
	return true;
}

void CCSPlayer::HandleTankDeathPostThink( float flFromCycle )
{
	if ( !m_bTankDeathInProgress )
		return;

	if ( GetSequence() != m_nTankDeathSequence )
	{
		ResetSequence( m_nTankDeathSequence );
		SetCycle( 0.0f );
		SetPlaybackRate( 1.0f );
		flFromCycle = 0.0f;
	}
	...
	if ( flToCycle != flFromCycle )
	{
		...
		{
			// Shouldn't happen for non-looping death sequences, but handle cycle wrap safely.
			Vector dp1, dp2;
			QAngle da1, da2;
	...
	if ( deltaPos != vec3_origin || deltaAngles.y != 0.0f )
	{
		...
		UTIL_TraceHull( start, end, WorldAlignMins(), WorldAlignMaxs(), MASK_SOLID, &filter, &tr );
...
void CCSPlayer::FinishTankDeath( void )
{
	if ( !m_bTankDeathInProgress )
		return;

	const CTakeDamageInfo info = m_TankDeathInfo;
	...
	// Restore normal player collision so the standard death code can run cleanly.
...
int CCSPlayer::OnTakeDamage_Alive( const CTakeDamageInfo &info )
{
	...
	// Shoves (DMG_CLUB) break hunter pounces.
	if ( ( info.GetDamageType() & DMG_CLUB ) != 0 && HasPounceVictim() )
	{
		ClearPounce();
	}
	...
	if ( TryStartTankDeath( info ) )
		return 1;

	if (GetTeamNumber() == 3) {
		if (info.GetDamageType() & DMG_BURN) {
			Ignite(90.0f, false, 0.0f, false);
			Scorch(8, 50);
		}
	}

	TryStartDamageStagger( info );

	// fire global game event

	IGameEvent * event = gameeventmanager->CreateEvent( "player_hurt" );
	if (GetTeamNumber() == 3) {
		EmitInfectedPainSound(info);
	}
	if (GetTeamNumber() == TEAM_SURVIVOR)
	{
		if (m_flNextPainSoundTime < gpGlobals->curtime)
		{
			...
			float maxHealth = (float)GetHealth();

			float damageRatio = damage / maxHealth;
			char responseContext[128];
			int intensity = 0;
			if (m_bIncapacitated)
			{
				V_strncat(responseContext, "PainLevel:Incapacitated", sizeof(responseContext), -1);
				intensity = 5;
			}
			else if (damageRatio < 0.2f)
			{
				V_strncat(responseContext, "PainLevel:Minor", sizeof(responseContext), -1);
				intensity = 2;
			}
			else if (damageRatio >= 0.5f)
			{
				V_strncat(responseContext, "PainLevel:Critical", sizeof(responseContext), -1);
				intensity = 4;
			}
			else
			{
				V_strncat(responseContext, "PainLevel:Major", sizeof(responseContext), -1);
				intensity = 3;
			}
			...
			if (info.GetDamageType() & DMG_BURN)
			{
				V_strncat(responseContext, ",DamageType:DMG_BURN", sizeof(responseContext), -1);
			}
			AddContext(responseContext);
			//---------------------------------------------------------
			// Speak pain concept (with cooldown)
			//---------------------------------------------------------
			...
			if (m_pounceAttacker != null)
			{
				...
				if (SpeakConceptIfAllowed(MP_CONCEPT_HURT, "", szResponse, AI_Response::MAX_RESPONSE_NAME))
				{
				...
				if (SpeakConceptIfAllowed(MP_CONCEPT_PLAYER_PAIN, "", szResponse, AI_Response::MAX_RESPONSE_NAME))
				{
					flPainLength = MAX(GetSceneDuration(szResponse), flPainLength);
				}
			...
			//---------------------------------------------------------
			// Remove burn flag if it was temporarily added
			//---------------------------------------------------------
			if (!(info.GetDamageType() & DMG_BURN))
			{
			...
			if (info.GetAttacker()->GetTeamNumber() == this->GetTeamNumber()) {
				SpeakConceptIfAllowed(MP_CONCEPT_PLAYER_ATTACKER_PAIN, "", szResponse, AI_Response::MAX_RESPONSE_NAME);
			}
	...
	if (info.GetAttacker() && info.GetAttacker()->IsPlayer()) {
		if (info.GetAttacker()->GetTeamNumber() == 3) {
			if (m_pounceAttacker == NULL)
			{
				EmitSound("PlayerZombie.AttackHit");
			}		
		}
	}

	CCSPlayer *pScoringAttacker = ToCSPlayer( info.GetAttacker() );
	if ( pScoringAttacker &&
		 pScoringAttacker != this &&
		 pScoringAttacker->GetTeamNumber() == TEAM_INFECTED &&
		 pScoringAttacker->IsSpecialInfected() &&
		 pScoringAttacker->GetZombieClass() > 0 &&
		 pScoringAttacker->GetTeamNumber() != GetTeamNumber() &&
		 m_lastDamageHealth > 0 )
	{
		pScoringAttacker->AddPoints( m_lastDamageHealth, true );
	}
	...
	if ( event )
	{
		...
		event->SetInt( "damageamount", m_lastDamageHealth );
...
void CreateExplosion(const CTakeDamageInfo& info, float innerRadius, float outerRadius)
{
	Vector explosionPos = info.GetDamagePosition();

	CBaseEntity* ent;
	for (CEntitySphereQuery sphere(explosionPos, outerRadius); (ent = sphere.GetCurrentEntity()) != NULL; sphere.NextEntity())
	{
		// --------------------------------------------------------------------
		...
		Vector closestPoint;
		ent->CollisionProp()->CalcNearestPoint(explosionPos, &closestPoint);

		Vector delta = closestPoint - explosionPos;
		float distance = delta.Length();

		if (distance > outerRadius)
			continue;

		float visibility = CSGameRules()->GetAmountOfEntityVisible(explosionPos, ent);
		if (visibility <= 0.0f)
			continue;
		...
		{
			falloff = visibility * ((distance - innerRadius) / (innerRadius - outerRadius) + 1.0f);
			if (falloff <= 0.0f)
				continue;
		}

		// --------------------------------------------------------------------
		// Build damage info copy
		// --------------------------------------------------------------------
		CTakeDamageInfo dmg = info;
		dmg.ScaleDamageForce(falloff);
		...
		// --------------------------------------------------------------------
		if (dmg.GetDamageForce() == vec3_origin)
		{
			CalculateExplosiveDamageForce(&dmg, dir, explosionPos, 1.5f);
		}
		else
		{
			float magnitude = dmg.GetDamageForce().Length();
			dmg.SetDamageForce(dir * magnitude * falloff);
			dmg.SetDamagePosition(explosionPos);
		}

		ent->TakeDamage(dmg);

		ent->TraceAttackToTriggers(dmg, explosionPos, closestPoint, dir);
		if (ent->IsPlayer() && ent->GetTeamNumber() == TEAM_SURVIVOR)
		{
			CSGameRules()->OnSurvivorVomited(ToCSPlayer(ent), NULL);
		}
	}

	DispatchParticleEffect("boomer_explode", explosionPos, vec3_angle);

	CBaseEntity* attacker = info.GetAttacker();
	if (attacker)
	{
		attacker->EmitSound("BoomerZombie.Detonate");
	}
...
void CCSPlayer::Event_Killed( const CTakeDamageInfo &info )
{
	...
	if ( m_pSpitterBurnMusic )
	{
		CSoundEnvelopeController::GetController().SoundFadeOut( m_pSpitterBurnMusic, 0.5f, true );
		m_pSpitterBurnMusic = NULL;
	}
	...
	if (pAttacker != NULL && pAttacker->GetTeamNumber() == TEAM_SURVIVOR) {
		SpeakConceptIfAllowed(MP_CONCEPT_KILLED_PLAYER);
	}
	//=============================================================================
	// HPE_END
	//=============================================================================
	...
	SetProgressBarTime( 0 );
	if (GetZombieClass() == 2 && GetTeamNumber() == 3) {
		SetModel("models/infected/limbs/exploded_boomer.mdl");
	}
	if (GetZombieClass() == 8 && GetTeamNumber() == 3) {
	...
	// Spitter death puddle: spawn a smaller area-denial acid pool.
	if ( GetTeamNumber() == TEAM_INFECTED && GetZombieClass() == 4 )
	{
		Vector acidPos = GetAbsOrigin();
		Vector acidNormal( 0, 0, 1 );

		trace_t ground;
		UTIL_TraceLine( acidPos + Vector( 0, 0, 16.0f ), acidPos - Vector( 0, 0, 256.0f ), MASK_SOLID_BRUSHONLY, this, COLLISION_GROUP_NONE, &ground );
		if ( ground.fraction < 1.0f )
		{
			acidPos = ground.endpos;
			acidNormal = ground.plane.normal;
		}

		if ( ( UTIL_PointContents( acidPos ) & MASK_WATER ) == 0 )
		{
			CInferno *inferno = (CInferno*)CBaseEntity::Create( "inferno", acidPos, QAngle( 0, 0, 0 ), this );
			if ( inferno )
			{
				inferno->SetOwnerEntity( this );
				inferno->SetInfernoType( INFERNO_TYPE_SPITTER_ACID );
				inferno->SetMaxFlames( 2 );
				inferno->StartBurning( acidPos, acidNormal, vec3_origin );
			}
		}
	}
	...
	const bool isSpecialInfected = ( GetTeamNumber() == TEAM_INFECTED && GetZombieClass() > 0 );
	if ( isSpecialInfected )
	{
		m_flSpecialInfectedDeathTimestamp = gpGlobals->curtime;
	}
...
void CCSPlayer::DeathSound( const CTakeDamageInfo &info )
{
		...
		if (GetTeamNumber() == 3) {
			// ------------------------------------------------------------------
			// Infected death sounds
			// ------------------------------------------------------------------
				// Do not play death sounds for ghosts
			switch (GetZombieClass())
			{
			case 1:
			{
				EmitSound("SmokerZombie.Death");
				EmitSound("SmokerZombie.Explode");
			...
			case 2:
			{
				...
				dmgInfo.SetDamageForce(RandomVector(-1,1) * 5000);
				dmgInfo.SetDamage(2);
				dmgInfo.SetDamageType(DMG_BLAST|DMG_POISON);
				dmgInfo.SetDamagePosition(GetAbsOrigin());
				//CreateExplosion(dmgInfo, 130, 200);
				EmitSound("Player.StopBody");
				EmitSound("Player.StopVoice");
				break;
			}
			...
			case 3:
			{
				EmitSound("HunterZombie.Death");
				break;
			}

			case 4:
			{
				EmitSound("SpitterZombie.Death");
				break;
			}

			case 5:
			{
				EmitSound("JockeyZombie.Death");
				break;
			}

			case 6:
			{
				EmitSound("ChargerZombie.Death");
				break;
			}
...
#define CS_SPITTER_BURN_MUSIC_THINK_CONTEXT "SpitterBurnMusicThink"

void CCSPlayer::NotifySpitterAcidBurned()
{
	if ( GetTeamNumber() != TEAM_SURVIVOR || !IsAlive() )
		return;

	m_flLastSpitterAcidDamageTime = gpGlobals->curtime;

	CSoundEnvelopeController &controller = CSoundEnvelopeController::GetController();
	if ( !m_pSpitterBurnMusic )
	{
		CSingleUserRecipientFilter filter( this );
		m_pSpitterBurnMusic = controller.SoundCreate( filter, entindex(), CHAN_STATIC, "Event.SpitterBurn", ATTN_NONE );
		if ( m_pSpitterBurnMusic )
		{
			controller.Play( m_pSpitterBurnMusic, 1.0f, 100 );
		}
	}

	SetContextThink( &CCSPlayer::SpitterBurnMusicThink, gpGlobals->curtime + 0.1f, CS_SPITTER_BURN_MUSIC_THINK_CONTEXT );
}

void CCSPlayer::SpitterBurnMusicThink()
{
	if ( !m_pSpitterBurnMusic )
		return;

	const float grace = 0.35f;
	if ( !IsAlive() || GetTeamNumber() != TEAM_SURVIVOR || ( gpGlobals->curtime - m_flLastSpitterAcidDamageTime ) > grace )
	{
		CSoundEnvelopeController::GetController().SoundFadeOut( m_pSpitterBurnMusic, 1.0f, true );
		m_pSpitterBurnMusic = NULL;
		return;
	}
...
void CCSPlayer::PostThink()
{
		...
		gpGlobals->curtime >= m_flNextWorldAntiStuckTime)
	{
		...
		trace_t tr;
		UTIL_TraceHull(GetAbsOrigin(), GetAbsOrigin(), mins, maxs, MASK_PLAYERSOLID_BRUSHONLY, this, COLLISION_GROUP_PLAYER_MOVEMENT, &tr);
		if (tr.startsolid || tr.allsolid)
		{
			const Vector anchor = (m_lastStandingPos != vec3_origin) ? m_lastStandingPos : GetAbsOrigin();
			if (TeleportPlayerToNearestNavArea(this, anchor))
			{
				m_flNextWorldAntiStuckTime = gpGlobals->curtime + 1.0f;
			}
		}
	}

	if (IsGhost() && IsAlive() && GetTeamNumber() == TEAM_INFECTED && GetZombieClass() > 0 &&
		m_flGhostMaterializeTime > 0.0f && gpGlobals->curtime >= m_flGhostMaterializeTime)
	{
		SetGhost(false);
	}
	...
	if (IsGhost() && IsAlive() && GetTeamNumber() == TEAM_INFECTED && GetZombieClass() > 0 &&
		m_flGhostMaterializeTime <= 0.0f &&
		gpGlobals->curtime >= m_flNextGhostStatusMessageTime)
	{
		UpdateGhostSpawnFeedback(true);
		m_flNextGhostStatusMessageTime = gpGlobals->curtime + kGhostStatusMessageInterval;
	}

	// Hunter pounce upkeep / damage ticking.
	if (HasPounceVictim())
	{
		CCSPlayer* pVictim = GetPounceVictim();
		if (!IsAlive() || !pVictim || !pVictim->IsAlive() || pVictim->GetTeamNumber() != TEAM_SURVIVOR || pVictim->GetPounceAttacker() != this)
		{
			ClearPounce();
		}
		else
		{
			// Pull the hunter towards the victim smoothly.
			const Vector currentOrigin = GetAbsOrigin();
			const Vector targetOrigin = pVictim->GetAbsOrigin();
			Vector toTarget = targetOrigin - currentOrigin;
			const float dist = toTarget.NormalizeInPlace();
			if (dist > 1.0f)
			{
				const float pullSpeed = 150.0f;
				const float step = MIN(dist, pullSpeed * gpGlobals->frametime);
				const Vector desiredOrigin = currentOrigin + toTarget * step;

				const Vector mins = m_Local.m_bDucked ? VEC_DUCK_HULL_MIN_SCALED(this) : VEC_HULL_MIN_SCALED(this);
				const Vector maxs = m_Local.m_bDucked ? VEC_DUCK_HULL_MAX_SCALED(this) : VEC_HULL_MAX_SCALED(this);

				trace_t tr;
				CTraceFilterSkipTwoEntities traceFilter(this, pVictim, COLLISION_GROUP_PLAYER_MOVEMENT);
				UTIL_TraceHull(currentOrigin, desiredOrigin, mins, maxs, MASK_PLAYERSOLID, &traceFilter, &tr);

				const Vector newOrigin = tr.endpos;
				Teleport(&newOrigin, NULL, &vec3_origin);
			}

			// After a short delay, deal damage at a fixed interval.
			while (gpGlobals->curtime >= m_flNextPounceDamageTime)
			{
				CTakeDamageInfo pounceInfo(this, this, 5.0f, DMG_SLASH | DMG_NEVERGIB);
				pVictim->TakeDamage(pounceInfo);
				...
				m_flNextPounceDamageTime += 0.5f;
			}
		}
	}
	else if (HasPounceAttacker())
	{
		CCSPlayer* pAttacker = GetPounceAttacker();
		if (!pAttacker || !pAttacker->IsAlive() || pAttacker->GetPounceVictim() != this)
		{
			ClearPounce();
		}
	}
	...
	// Charger upkeep: charge movement, carry, wall impacts, and pummel sequences.
	{
		...
		if (HasChargerVictim())
		{
			...
			{
				PhysDisableEntityCollisions(this, victim);
			}
		}
		else if (HasChargerAttacker())
		{
			...
			{
				PhysDisableEntityCollisions(attacker, this);
			}
		}
		...
		auto StartChargerSlam = [&]()
			{
				...
				if (victim)
				{
					...
					KeepChargerVictimAttached(this, victim, GetAbsOrigin());
				}
		...
		if (GetTeamNumber() == TEAM_INFECTED && GetZombieClass() == 6 && m_nChargerAction == CHARGER_ACTION_CHARGING)
		{
			...
			else
			{
				...
				const Vector curPos = GetAbsOrigin() + Vector(0, 0, 64.0f);
				Vector traceStart = m_vecChargerTracePos;
				if (traceStart == vec3_origin)
				{
					traceStart = curPos;
				}
				...
				CCSPlayer* sweptVictim = NULL;
				float flSweptVictimFraction = 1.0f;
				CUtlVector< CCSPlayer* > sweptOtherVictims;
				CollectChargerChargeVictimsAlongSweep(this, traceStart, nextPos, mins, maxs, &sweptVictim, &flSweptVictimFraction, &sweptOtherVictims);

				CTraceFilterSkipTwoEntities filter(this, GetChargerVictim(), COLLISION_GROUP_NONE);
				UTIL_TraceHull(traceStart, nextPos, mins, maxs, MASK_SHOT, &filter, &tr);
				m_vecChargerTracePos = curPos;

				const bool bHitWall = (tr.startsolid || tr.allsolid || tr.fraction < 1.0f);
				const float flWallFraction = (tr.startsolid || tr.allsolid) ? 0.0f : tr.fraction;
				const bool bVictimBeforeWall = (sweptVictim != NULL) && (!bHitWall);

				// Survivor impacts: pick up the first target, blast others.
				if (bVictimBeforeWall)
				{
					CCSPlayer* hitPlayer = sweptVictim;
					if (hitPlayer)
					{
						if (!HasChargerVictim() && !m_bChargerChargeCapturedVictim && !hitPlayer->HasChargerAttacker())
						{
							m_chargerVictim = hitPlayer;
							m_bChargerChargeCapturedVictim = true;
							...
							KeepChargerVictimAttached(this, hitPlayer, GetAbsOrigin());
							EmitSound("ChargerZombie.HitPerson");
						}
						else
						{
					...
					for (int i = 0; i < sweptOtherVictims.Count(); ++i)
					{
						BlastBystander(sweptOtherVictims[i]);
					}
				}
				else if (bHitWall)
				{
					...
					else
					{
						...
						DispatchParticleEffect("charger_wall_impact", GetAbsOrigin(), GetAbsAngles());
	...
	if (GetTeamNumber() == TEAM_INFECTED && GetZombieClass() == 2 && IsAlive())
	{
		if (GetAbsVelocity().z < 0.0f)
		{
			float now = gpGlobals->curtime;
	...
	if ( IsAlive() && m_nDamageStaggerDir != PLAYER_STAGGER_DIR_NONE &&
		 GetMoveType() == MOVETYPE_NONE && GetSolid() != SOLID_NONE )
	{
		const float flFromCycle = m_flDamageStaggerPrevCycle;
		const float flToCycle = GetCycle();
		...
		if ( deltaPos != vec3_origin || deltaAngles.y != 0.0f )
		{
			...
			trace_t tr;
			CTraceFilterSimple filter( this, COLLISION_GROUP_PLAYER_MOVEMENT );
			UTIL_TraceHull( start, end, WorldAlignMins(), WorldAlignMaxs(), MASK_PLAYERSOLID, &filter, &tr );
		...
		m_flDamageStaggerPrevCycle = flToCycle;
	...
	// Survivor stomp: stepping on a common infected instantly kills it.
	if ( IsAlive() && GetTeamNumber() == TEAM_SURVIVOR )
	{
		CBaseEntity *ground = GetGroundEntity();
		if ( ground && FClassnameIs( ground, "infected" ) )
		{
			CBaseCombatCharacter *infected = dynamic_cast< CBaseCombatCharacter * >( ground );
			if ( infected && infected->IsAlive() )
			{
				CTakeDamageInfo info( this, this, 9999.0f, DMG_CRUSH | DMG_NEVERGIB );
				infected->TakeDamage( info );
			}
		}
	}

	// Bots should have infinite ammo (but not infinite grenades/C4/knife).
	if ( IsAlive() && IsBot() )
	{
		...
		if ( ammoDef )
		{
			for ( int i = 0; i < MAX_WEAPONS; ++i )
			{
				...
				const CSWeaponType weaponType = weapon->GetCSWpnData().m_WeaponType;
				if ( weaponType == WEAPONTYPE_GRENADE || weaponType == WEAPONTYPE_KNIFE || weaponType == WEAPONTYPE_C4 )
					continue;

				const int ammo1 = weapon->GetPrimaryAmmoType();
				if ( ammo1 >= 0 )
				{
					const int maxCarry = ammoDef->MaxCarry( ammo1 );
					if ( maxCarry > 0 && GetAmmoCount( ammo1 ) < maxCarry )
					{
						SetAmmoCount( maxCarry, ammo1 );
					}
				}

				const int ammo2 = weapon->GetSecondaryAmmoType();
				if ( ammo2 >= 0 )
				{
					const int maxCarry = ammoDef->MaxCarry( ammo2 );
					if ( maxCarry > 0 && GetAmmoCount( ammo2 ) < maxCarry )
					{
						SetAmmoCount( maxCarry, ammo2 );
					}
				}
			}
		}
...
bool CCSPlayer::Weapon_CanSwitchTo( CBaseCombatWeapon *pWeapon )
{
	...
	if ( m_bIncapacitated )
	{
		// While incapacitated, survivors can only use their pistol.
		if ( pWeapon->GetSlot() != WEAPON_SLOT_PISTOL )
			return false;
	}
...
//=============================================================================
		...
		if ( GetTeamNumber() == TEAM_SURVIVOR && IsAlive() && !m_bIncapacitated )
		{
			...
			if ( dmg > 0.0f && dmg >= (float)GetHealth() )
			{
				if ( m_nIncapacitationCount < 2 )
				{
					...
					if (SpeakConceptIfAllowed(MP_CONCEPT_STUNNED, "", szResponse, AI_Response::MAX_RESPONSE_NAME))
					{
						flPainLength = MAX(GetSceneDuration(szResponse), flPainLength);
					}
					...
					SpeakConceptIfAllowed(MP_CONCEPT_TIRED);
					SetHealth( 300 );

					// Force pistol out if possible.
					CBaseCombatWeapon *pistol = Weapon_GetSlot( WEAPON_SLOT_PISTOL );
					if ( pistol )
					{
...
static bool IsValidIncapReviveTarget( CCSPlayer *reviver, CCSPlayer *target )
{
	if ( !reviver || !target || target == reviver )
		return false;

	if ( target->GetTeamNumber() != TEAM_SURVIVOR || !target->IsAlive() || !target->IsIncapacitated() )
		return false;

	const float maxDistSqr = ( 128.0f * 128.0f );
	if ( ( target->WorldSpaceCenter() - reviver->WorldSpaceCenter() ).LengthSqr() > maxDistSqr )
		return false;

	return true;
}

static bool CanUseToReviveTarget( CCSPlayer *reviver, CCSPlayer *target, const Vector &forward, bool requireFacing )
{
	if ( !IsValidIncapReviveTarget( reviver, target ) )
		return false;

	Vector toTarget = target->WorldSpaceCenter() - reviver->EyePosition();
	...
	trace_t tr;
	UTIL_TraceLine( reviver->EyePosition(), target->WorldSpaceCenter(), MASK_SHOT, reviver, COLLISION_GROUP_NONE, &tr );
	if ( tr.m_pEnt == target || tr.fraction >= 0.99f )
		return true;

	UTIL_TraceLine( reviver->EyePosition(), target->EyePosition(), MASK_SHOT, reviver, COLLISION_GROUP_NONE, &tr );
	return tr.m_pEnt == target || tr.fraction >= 0.99f;
}

static CCSPlayer *FindIncapReviveTarget( CCSPlayer *reviver )
{
	if ( !reviver )
		return NULL;

	Vector forward;
	AngleVectors( reviver->EyeAngles(), &forward );

	const Vector start = reviver->EyePosition();
	const Vector end = start + ( forward * 96.0f );

	trace_t tr;
	UTIL_TraceLine( start, end, MASK_SHOT, reviver, COLLISION_GROUP_NONE, &tr );

	CCSPlayer *target = ToCSPlayer( tr.m_pEnt );
	if ( CanUseToReviveTarget( reviver, target, forward, true ) )
		return target;
	...
	{
		CCSPlayer *player = ToCSPlayer( UTIL_PlayerByIndex( i ) );
		if ( !CanUseToReviveTarget( reviver, player, forward, true ) )
			continue;
...
void CCSPlayer::PreThink()
{
	...
	// Infected should always have the flashlight on.
	if ( GetTeamNumber() == TEAM_INFECTED && IsAlive() && !FlashlightIsOn() )
	{
		FlashlightTurnOn();
	}
	...
	if ( GetTeamNumber() == TEAM_SURVIVOR && IsAlive() )
	{
		...
		if ( m_bIncapHealthDecayActive )
		{
			...
			while ( gpGlobals->curtime >= m_flNextIncapHealthDecayTime )
			{
				const int newHealth = GetHealth() - 1;
				SetHealth( newHealth );
				m_flNextIncapHealthDecayTime += 4.0f;
		...
		if ( m_bIncapacitated )
		{
			if ( m_flNextIncapHelpYellTime <= 0.0f )
			{
				m_flNextIncapHelpYellTime = gpGlobals->curtime + 15.0f;
			}

			if ( gpGlobals->curtime >= m_flNextIncapHelpYellTime )
			{
				// Yell for help every 15 seconds while incapacitated.
	...
	// Revive incapacitated teammates: hold +use for 5 seconds.
	if ( GetTeamNumber() == TEAM_SURVIVOR && IsAlive() && !m_bIncapacitated )
	{
		const bool holdingUse = ( ( m_nButtons & IN_USE ) != 0 );
		CCSPlayer *target = NULL;
		if ( holdingUse )
		{
			Vector forward;
			AngleVectors( EyeAngles(), &forward );

			CCSPlayer *currentTarget = ToCSPlayer( m_hReviveTarget.Get() );
			if ( CanUseToReviveTarget( this, currentTarget, forward, false ) )
			{
				target = currentTarget;
			}
			else
			{
				target = FindIncapReviveTarget( this );
			}
		}
		...
		if ( holdingUse && target )
		{
			...
			if ( m_hReviveTarget.Get() != target )
			{
				if (target->m_nIncapacitationCount > 1) {
					SpeakConceptIfAllowed(MP_CONCEPT_PLAYER_SHOW_ITEM_TAUNT);
				}
				else {
					SpeakConceptIfAllowed(MP_CONCEPT_DUEL_ACCEPTED);
				}
				m_hReviveTarget = target;
				m_flReviveStartTime = gpGlobals->curtime;
				SetProgressBarTime( 5 );
			}

			if ( m_flReviveStartTime > 0.0f && ( gpGlobals->curtime - m_flReviveStartTime ) >= 5.0f )
			{
				CCSPlayer *victim = ToCSPlayer( m_hReviveTarget.Get() );
				if ( victim && victim->IsAlive() && victim->IsIncapacitated() && victim->GetTeamNumber() == TEAM_SURVIVOR )
				{
					...
					victim->SetHealth( 30 );
					victim->SpeakConceptIfAllowed(MP_CONCEPT_PLAYER_THANKS);

					// After the 2nd incap + revive, mark them black-and-white (client handles visuals/audio).
					if ( victim->m_nIncapacitationCount >= 2 )
					{
						victim->m_bIncapBlackAndWhite = true;
					}
				}
	...
	if ( mp_autokick.GetBool() && !IsBot() && !IsHLTV() && !IsAutoKickDisabled() && !CSGameRules()->IsLogoMap() )
	{
...
void CCSPlayer::SetZombieClass(int zombieClass)
{
	m_zombieClass = zombieClass;
	if (IsHulkTank() && !m_bTankMusicCounted)
	{
		m_bTankMusicCounted = true;
		TankMusic_AddTank();
	}
}

void CCSPlayer::StartPounce( CCSPlayer *pVictim )
{
	if ( !pVictim || pVictim == this )
		return;

	// Only hunters can pounce.
	if ( GetTeamNumber() != TEAM_INFECTED || GetZombieClass() != 3 )
		return;

	if ( !IsAlive() || !pVictim->IsAlive() )
		return;

	if ( pVictim->GetTeamNumber() != TEAM_SURVIVOR )
		return;

	// Ensure we don't keep stale links around.
	ClearPounce();

	// If the victim is already pounced, clear that relationship first.
	if ( pVictim->HasPounceAttacker() )
	{
		CCSPlayer *pExistingAttacker = pVictim->GetPounceAttacker();
		if ( pExistingAttacker )
		{
			pExistingAttacker->ClearPounce();
		}
		else
		{
			pVictim->m_pounceAttacker = NULL;
		}
	}

	m_pounceVictim = pVictim;
	pVictim->m_pounceAttacker = this;

	m_flPounceStartTime = gpGlobals->curtime;
	m_flNextPounceDamageTime = gpGlobals->curtime + 1.0f;

	// Stop momentum on both players so the pin feels stable.
	SetAbsVelocity( vec3_origin );
	pVictim->SetAbsVelocity( vec3_origin );

	// Attacker and victim should not collide with each other while pounced.
	PhysDisableEntityCollisions( this, pVictim );
}

void CCSPlayer::ClearPounce( void )
{
	CCSPlayer *pVictim = m_pounceVictim.Get();
	if ( pVictim )
	{
		PhysEnableEntityCollisions( this, pVictim );

		if ( pVictim->m_pounceAttacker.Get() == this )
		{
			pVictim->m_pounceAttacker = NULL;
		}
		m_pounceVictim = NULL;
	...
	CCSPlayer *pAttacker = m_pounceAttacker.Get();
	if ( pAttacker )
	{
		PhysEnableEntityCollisions( pAttacker, this );

		if ( pAttacker->m_pounceVictim.Get() == this )
		{
			pAttacker->m_pounceVictim = NULL;
			pAttacker->m_flPounceStartTime = 0.0f;
			pAttacker->m_flNextPounceDamageTime = 0.0f;
		}
		m_pounceAttacker = NULL;
	}
...
void CCSPlayer::StartChargerCharge()
{
	...
	m_flChargerPrevCycle = 0.0f;
	m_vecChargerTracePos = GetAbsOrigin() + Vector( 0, 0, 64.0f );
	m_ChargerChargeHitVictims.Purge();
	m_bChargerChargeCapturedVictim = false;
...
void CCSPlayer::ClearCharger( void )
{
	...
	if ( attacker )
	{
		...
		if ( attacker->m_chargerVictim.Get() == this )
		{
			...
			attacker->m_vecChargerTracePos = vec3_origin;
			attacker->m_ChargerChargeHitVictims.Purge();
			attacker->m_bChargerChargeCapturedVictim = false;
			if ( gpGlobals && attacker->IsAlive() )
			{
				attacker->m_flNextChargerChargeAllowedTime = MAX( attacker->m_flNextChargerChargeAllowedTime, gpGlobals->curtime + 12.0f );
			}
	...
	if ( GetTeamNumber() == TEAM_INFECTED && GetZombieClass() == 6 )
	{
		if ( m_nChargerAction != CHARGER_ACTION_NONE )
		{
			if ( gpGlobals && IsAlive() )
			{
				m_flNextChargerChargeAllowedTime = MAX( m_flNextChargerChargeAllowedTime, gpGlobals->curtime + 12.0f );
			}
			m_nChargerAction = CHARGER_ACTION_NONE;
			m_nChargerStaggerDir = CHARGER_STAGGER_DIR_NONE;
			m_flChargerPrevCycle = 0.0f;
...
void CCSPlayer::ClearDamageStagger( void )
{
	m_nDamageStaggerDir = PLAYER_STAGGER_DIR_NONE;
	m_flDamageStaggerPrevCycle = 0.0f;
	m_angDamageStaggerEyeAngles.Init();
	...
	if ( IsAlive() && GetMoveType() == MOVETYPE_NONE &&
		 !HasPounceVictim() && !HasPounceAttacker() &&
		 !HasChargerVictim() && !HasChargerAttacker() &&
		 m_nChargerAction == CHARGER_ACTION_NONE &&
		 m_nTankAction == TANK_ACTION_NONE &&
		 GetSolid() != SOLID_NONE )
	{
...
bool CCSPlayer::TryStartDamageStagger( const CTakeDamageInfo &info )
{
	if ( !IsAlive() )
		return false;

	const int dmgType = info.GetDamageType();
	const bool shoveByBlast = ( dmgType & DMG_BLAST ) != 0;
	bool shoveByClub = ( dmgType & DMG_CLUB ) != 0;

	if ( shoveByClub && GetTeamNumber() == TEAM_INFECTED )
	{
		const int zclass = GetZombieClass();
		if ( zclass == 6 || zclass == 8 )
		{
			shoveByClub = false;
		}
	}

	if ( !shoveByBlast && !shoveByClub )
		return false;

	const Vector shoveDir = ComputeDamageShoveDir( this, info );
	if ( shoveDir == vec3_origin )
		return false;

	Activity activity = ACT_INVALID;
	PlayerStaggerDir_t dir = ComputePlayerStaggerDir( this, shoveDir, &activity );
	if ( dir == PLAYER_STAGGER_DIR_NONE || activity == ACT_INVALID )
		return false;

	int seq = SelectWeightedSequence( activity );
	if ( seq == -1 && dir != PLAYER_STAGGER_DIR_BACK )
	{
		dir = PLAYER_STAGGER_DIR_BACK;
		activity = ACT_TERROR_SHOVED_BACKWARD;
		seq = SelectWeightedSequence( activity );
	}
	...
	if ( HasChargerVictim() || HasChargerAttacker() ||
		 ( GetTeamNumber() == TEAM_INFECTED && GetZombieClass() == 6 && m_nChargerAction != CHARGER_ACTION_NONE ) )
	{
		ClearCharger();
	}
	...
	m_nDamageStaggerDir = dir;
	m_flDamageStaggerPrevCycle = 0.0f;
	m_angDamageStaggerEyeAngles = EyeAngles();
	m_angDamageStaggerEyeAngles.z = 0.0f;
...
void CCSPlayer::MoveToNextIntroCamera()
{
	...
	m_fIntroCamTime = gpGlobals->curtime + 30.f;
}
...
{
	if (GetTeamNumber() == TEAM_INFECTED) return false;

	// Survivor bots should be able to buy immediately on spawn even on maps without buy zones / buy time.
	if ( IsBot() && GetTeamNumber() == TEAM_SURVIVOR )
	{
...
namespace
{
	struct SurvivorBotTakeoverWeapon
	{
		char classname[64];
		int clip1;
		int clip2;
	...
	struct SurvivorBotTakeoverSnapshot
	{
		Vector origin;
		QAngle angles;
		Vector velocity;
		...
		CUtlVector< SurvivorBotTakeoverWeapon > weapons;
	};

	static void ApplySurvivorArmsModel( CCSPlayer *player )
	{
		if ( !player || player->GetTeamNumber() != TEAM_SURVIVOR )
			return;

		CBaseViewModel *vm = player->GetViewModel( 1 );
		if ( !vm )
			return;

		switch ( player->GetSurvivorClass() )
		{
			case 0: vm->SetModel( "models/weapons/arms/v_arms_gambler_new.mdl" ); break;
			case 1: vm->SetModel( "models/weapons/arms/v_arms_coach_new.mdl" ); break;
			case 2: vm->SetModel( "models/weapons/arms/v_arms_mechanic_new.mdl" ); break;
			case 3: vm->SetModel( "models/weapons/arms/v_arms_producer_new.mdl" ); break;
			default: break;
		}
	}

	static void SnapshotForSurvivorBotTakeover( CCSPlayer *player, SurvivorBotTakeoverSnapshot *out )
	{
		if ( !player || !out )
			return;

		out->origin = player->GetAbsOrigin();
		out->angles = player->GetAbsAngles();
		out->velocity = player->GetAbsVelocity();

		out->health = player->GetHealth();
		out->maxHealth = player->GetMaxHealth();
		out->armor = player->ArmorValue();
		out->hasHelmet = player->m_bHasHelmet;
		out->account = player->m_iAccount;

		out->survivorClass = player->GetSurvivorClass();

		out->activeWeapon[0] = '\0';
		if ( CBaseCombatWeapon *active = player->GetActiveWeapon() )
		{
			V_strncpy( out->activeWeapon, active->GetClassname(), sizeof( out->activeWeapon ) );
		}

		for ( int i = 0; i < MAX_AMMO_TYPES; ++i )
		{
			out->ammo[i] = player->GetAmmoCount( i );
		}

		out->weapons.RemoveAll();
		out->weapons.EnsureCapacity( 8 );
		...
		for ( int i = 0; i < player->WeaponCount(); ++i )
		{
			...
			SurvivorBotTakeoverWeapon snap;
			V_strncpy( snap.classname, weapon->GetClassname(), sizeof( snap.classname ) );
			snap.clip1 = weapon->Clip1();
			snap.clip2 = weapon->Clip2();
			out->weapons.AddToTail( snap );
		}
	}

	static CCSBot *CreateSurvivorBotTakeover( void )
	{
		if ( !TheBotProfiles )
			return NULL;

		if ( !CSGameRules() || CSGameRules()->TeamFull( TEAM_SURVIVOR ) )
			return NULL;

		const BotDifficultyType difficulty = CCSBotManager::GetDifficultyLevel();
		const BotProfile *profile = TheBotProfiles->GetRandomProfile( difficulty, TEAM_SURVIVOR, WEAPONTYPE_UNKNOWN );
		if ( !profile )
			return NULL;

		CCSBot *bot = CreateBot< CCSBot >( profile, TEAM_SURVIVOR );
		if ( !bot )
			return NULL;

		bot->ChangeTeam( TEAM_SURVIVOR );
		bot->RoundRespawn();
		return bot;
	}

	static void ApplySurvivorBotTakeoverSnapshot( CCSBot *bot, const SurvivorBotTakeoverSnapshot &snap )
	{
		if ( !bot )
			return;

		// Cancel any pending bot buying; we'll restore the human's current loadout.
		bot->Idle();

		bot->RemoveAllItems( true );

		// Restore survivor identity/arms first.
		bot->SetSurvivorClass( snap.survivorClass );
		ApplySurvivorArmsModel( bot );

		bot->m_iAccount = snap.account;
		bot->m_bHasHelmet = snap.hasHelmet;
		bot->SetArmorValue( snap.armor );
		bot->SetMaxHealth( snap.maxHealth );
		bot->SetHealth( snap.health );

		// Restore ammo reserves before/after giving weapons (harmless either way).
		for ( int i = 0; i < MAX_AMMO_TYPES; ++i )
		{
			bot->SetAmmoCount( snap.ammo[i], i );
		}

		// Restore weapons and clip counts.
		for ( int i = 0; i < snap.weapons.Count(); ++i )
		{
			const SurvivorBotTakeoverWeapon &w = snap.weapons[i];
			CBaseEntity *ent = bot->GiveNamedItem( w.classname );
			CBaseCombatWeapon *weapon = dynamic_cast< CBaseCombatWeapon * >( ent );
			if ( weapon )
			{
				weapon->m_iClip1 = w.clip1;
				weapon->m_iClip2 = w.clip2;
			}
		}

		// Re-apply reserve ammo in case GiveNamedItem altered it.
		for ( int i = 0; i < MAX_AMMO_TYPES; ++i )
		{
			bot->SetAmmoCount( snap.ammo[i], i );
		}

		// Try to restore active weapon.
		if ( snap.activeWeapon[0] )
		{
			if ( CBaseCombatWeapon *active = bot->Weapon_OwnsThisType( snap.activeWeapon ) )
			{
				bot->Weapon_Switch( active );
			}
		}

		Vector vel = snap.velocity;
		bot->Teleport( &snap.origin, &snap.angles, &vel );
	}
}
...
void CCSPlayer::UpdateZombieSounds()
{
	// Only infected team
	if (GetTeamNumber() != 3)
		return;

	// Must be alive and not ghost
	if (!IsAlive())
		return;
	...
	int activityState = /*m_activityState*/0;

	float now = CountdownTimer().Now();

	static ConVar zombie_rage_near_dist( "zombie_rage_near_dist", "600", FCVAR_GAMEDLL, "Distance (units). If an infected sees an enemy within this range, rageLevel becomes 2; otherwise 1 when seen." );

	const int classType = GetZombieClass();

	// Track whether we can currently see a survivor, regardless of any other vocal cooldowns.
	CCSPlayer *closestVisibleSurvivor = NULL;
	...
	for ( int i = 1; i <= gpGlobals->maxClients; ++i )
	{
		...
		if ( survivor->GetTeamNumber() != TEAM_SURVIVOR )
			continue;

		if ( !survivor->IsAlive() )
			continue;

		if ( !FInViewCone( survivor ) )
			continue;

		if ( !FVisible( survivor ) )
			continue;

		const float distSqr = ( survivor->GetAbsOrigin() - GetAbsOrigin() ).LengthSqr();
		if ( distSqr < closestDistSqr )
		{
			closestDistSqr = distSqr;
			closestVisibleSurvivor = survivor;
		}
	}

	const bool seesEnemyNow = ( closestVisibleSurvivor != NULL );

	// Special infected recognition: when we spot an enemy, play a recognize sound once,
	// with a 10s cooldown while alive.
	if ( classType > 0 && seesEnemyNow )
	{
		const float recognizeCooldown = 10.0f;
		if ( !m_bWasSeeingEnemyLastUpdate && gpGlobals->curtime >= m_flNextRecognizeSoundTime )
		{
			const char *recognizeSound = NULL;
			switch ( classType )
			{
			case 1: recognizeSound = "SmokerZombie.Recognize"; break;
			case 4: recognizeSound = "SpitterZombie.Recognize"; break;
			case 5: recognizeSound = "JockeyZombie.Alert"; break;
			case 6: recognizeSound = "ChargerZombie.Recognize"; break;

			// Fallbacks for classes without a dedicated recognize sound.
			case 2: recognizeSound = "BoomerZombie.Rage"; break;
			case 3: recognizeSound = "HunterZombie.Warn"; break;
			case 8: recognizeSound = "HulkZombie.Yell"; break;
			}

			if ( recognizeSound )
			{
				EmitSound( recognizeSound );
				m_flNextRecognizeSoundTime = gpGlobals->curtime + recognizeCooldown;
			}
		}
	}

	m_bWasSeeingEnemyLastUpdate = seesEnemyNow;

	// rageLevel depends on whether we can see a survivor, and how close they are.
	int rageLevel = 0;
	if ( seesEnemyNow )
	{
		const float nearDist = zombie_rage_near_dist.GetFloat();
		rageLevel = ( closestDistSqr <= ( nearDist * nearDist ) ) ? 2 : 1;
	}


	if (GetTeamNumber() == TEAM_INFECTED && classType != 8)
	{
		if (gpGlobals->curtime >= m_nextVocalizeTime)
		{
			auto weapon = GetActiveWeapon();

			if (weapon && weapon->m_flNextSecondaryAttack >= gpGlobals->curtime)
			{
				switch (classType)
				{
				case 2:
	...
	// Global cooldown (for non-recognize vocalizations)
	if ( now < m_nextVocalizeTime || now < m_nextZombieSoundTime )	
		return;
	...
	case 1:
	{
		/*if (HasTongueVictim())
		{
			...
			m_zombieSoundTimer.Start(delay);
			return;
		}*/
	...
	case 8:
	{
		//if (IsThrowingRock() || HasPunchVictim())
			//return;
	...
	m_nextZombieSoundTime = now + delay;
	//----------------------------------------------------
	// Attack vocalizations
	//----------------------------------------------------
}

void CCSPlayer::UpdateSurvivorWarnSpecial()
{
	// Only survivors.
	if ( GetTeamNumber() != TEAM_SURVIVOR )
		return;

	if ( !IsAlive() || m_bIncapacitated )
		return;

	// Find the closest visible special infected.
	CCSPlayer *closestVisibleSpecial = NULL;
	float closestDistSqr = FLT_MAX;
	for ( int i = 1; i <= gpGlobals->maxClients; ++i )
	{
		CCSPlayer *infected = ToCSPlayer( UTIL_PlayerByIndex( i ) );
		...
		const int zclass = infected->GetZombieClass();
		if ( zclass <= 0 )
			continue;

		if ( !FInViewCone( infected ) )
			continue;

		if ( !FVisible( infected ) )
			continue;

		const float distSqr = ( infected->GetAbsOrigin() - GetAbsOrigin() ).LengthSqr();
		if ( distSqr < closestDistSqr )
		{
			closestDistSqr = distSqr;
			closestVisibleSpecial = infected;
		}
	}

	const bool seesSpecialNow = ( closestVisibleSpecial != NULL );
	const int currentSpecialType = seesSpecialNow ? closestVisibleSpecial->GetZombieClass() : 0;

	// Warn on acquisition (or when the visible special type changes), with a 10s cooldown while alive.
	if ( currentSpecialType > 0 )
	{
		const float warnCooldown = 5.0f;
		const bool specialChanged = ( currentSpecialType != m_nLastWarnedSpecialType );
		if ( ( !m_bWasSeeingEnemyLastUpdate || specialChanged ) && gpGlobals->curtime >= m_flNextRecognizeSoundTime )
		{
			const char *specialTypeName = NULL;
			switch ( currentSpecialType )
			{
			case 1: specialTypeName = "Smoker"; break;
			case 2: specialTypeName = "Boomer"; break;
			case 3: specialTypeName = "Hunter"; break;
			case 4: specialTypeName = "Spitter"; break;
			case 5: specialTypeName = "Jockey"; break;
			case 6: specialTypeName = "Charger"; break;
			case 8: specialTypeName = "Tank"; break;
			}
			...
			if ( specialTypeName )
			{
				char responseContext[128];
				responseContext[0] = 0;
				V_strncat( responseContext, "SpecialType:", sizeof( responseContext ), -1 );
				V_strncat( responseContext, specialTypeName, sizeof( responseContext ), -1 );
				AddContext( responseContext );
			}

			SpeakConceptIfAllowed( GetMPConceptIndexFromString( "PlayerWarnSpecial" ) );
			m_flNextRecognizeSoundTime = gpGlobals->curtime + warnCooldown;
			m_nLastWarnedSpecialType = currentSpecialType;
		}
	}
	else
	{
	...
	m_bWasSeeingEnemyLastUpdate = seesSpecialNow;
}

void CCSPlayer::EmitInfectedPainSound(const CTakeDamageInfo& dmgInfo)
{
	float now = gpGlobals->curtime;

	// Pain sound cooldown
	if (now < m_nextPainSoundTime)
		return;

	// Reset timers if needed
	if (m_nextPainSoundTime != now + 0.7f)
	{
		m_painTimer.Start(now);
		m_nextPainSoundTime = now + 0.7f;
	}

	if (m_painSoundDuration != 0.7f)
	{
		m_painTimer.Start(m_painSoundDuration);
		m_painSoundDuration = 0.7f;
	}

	bool shortPain = (dmgInfo.GetDamageType() & DMG_BURN) == 0;
...
void CCSPlayer::Vocalize(const char* soundName, float delay, float radius)
{
	...
	// Case 1: negative delay → play immediately, no cooldown tracking
	if (delay < 0.0f)
	{
		EmitSound(soundName);
	}

	// Case 2: delay == 0 → automatically determine cooldown from sound length
	else if (delay == 0.0f)
	{
		EmitSound(soundName);
		duration = GetSoundDuration(soundName, STRING(GetModelName()));
		cooldown = (duration < delay) ? delay + 0.2f : duration + 0.2f;

		if ((now + cooldown) != m_nextVocalizeTime)
		{
			NetworkStateChanged(&m_nextVocalizeTime);
			m_nextVocalizeTime = now + cooldown;
		}

		if (cooldown != m_vocalizeCooldown)
		{
			NetworkStateChanged(&m_vocalizeCooldown);
			m_vocalizeCooldown = cooldown;
		}
	}

	// Case 3: delay > 0 → explicit cooldown
	else
	{
		EmitSound(soundName);

		if ((now + delay) != m_nextVocalizeTime)
		{
			NetworkStateChanged(&m_nextVocalizeTime);
			m_nextVocalizeTime = now + delay;
		}

		if (delay != m_vocalizeCooldown)
		{
			NetworkStateChanged(&m_vocalizeCooldown);
...
bool HandleRadioAliasCommands( CCSPlayer *pPlayer, const char *pszCommand )
{
	...
	if ( FStrEq( pszCommand, "coverme" ) )
	{
		...
		pPlayer->SpeakConceptIfAllowed(MP_CONCEPT_PLAYER_MEDIC);
	}
	...
	else if ( FStrEq( pszCommand, "takingfire" ) )
	{
		...
		pPlayer->SpeakConceptIfAllowed(MP_CONCEPT_PLAYER_HELP);
	}
	// Radio2 commands
...
bool CCSPlayer::HandleCommand_JoinTeam( int team )
{
	...
	*/
	...
	if ( team == TEAM_SPECTATOR )
	{
		...
		{
			// If a living human survivor goes spectator, keep their character in the world by
			// letting ChangeTeam() hand it off to a survivor bot instead of forcing a suicide first.
			if ( !( !IsBot() && !IsDead() && GetTeamNumber() == TEAM_SURVIVOR ) )
			{
...
ReturnSpot:
	if ( !pSpot )
	{
		...
		if (SelectSpawnSpot("info_player_teamspawn", pSpot)) {
			return pSpot;
		}
...
// Purpose: 
//-----------------------------------------------------------------------------
void CCSPlayer::InputSpeakResponseConcept(inputdata_t& inputdata)
{
...
void CCSPlayer::State_PreThink_DEATH_ANIM()
{
	...
	{
		if ( gpGlobals->curtime < fFreezeEnd && GetObserverMode() != OBS_MODE_FREEZECAM )
		{
...
{
	CWeaponCSBase *pCSWeapon = dynamic_cast<CWeaponCSBase*>(pBaseWeapon);
	// Special infected should never pick up dropped weapons.
	if ( GetTeamNumber() == TEAM_INFECTED && !pCSWeapon->IsA(WEAPON_KNIFE))
		return false;
...
{
	if( ( flashlight.GetInt() > 0 || GetTeamNumber() == TEAM_INFECTED ) && IsAlive() )
	{
...
{
	// Infected cannot turn the flashlight off while alive.
	if ( GetTeamNumber() == TEAM_INFECTED && IsAlive() )
		return;
...
void CCSPlayer::ChangeTeam( int iTeamNum )
{
	...
	const bool bWasAlive = ( m_lifeState == LIFE_ALIVE );

	SurvivorBotTakeoverSnapshot survivorTakeover;
	const bool bShouldSurvivorBotTakeover =
		( !IsBot() &&
		  bWasAlive &&
		  iOldTeam == TEAM_SURVIVOR &&
		  iTeamNum != iOldTeam &&
		  iOldTeam != TEAM_UNASSIGNED &&
		  iOldTeam != TEAM_SPECTATOR );

	if ( bShouldSurvivorBotTakeover )
	{
		SnapshotForSurvivorBotTakeover( this, &survivorTakeover );
	}
	...
	if ( m_bTankMusicCounted && iTeamNum != TEAM_INFECTED )
	{
		TankMusic_RemoveTank();
		m_bTankMusicCounted = false;
	}
	...
	if ( bShouldSurvivorBotTakeover )
	{
		// Prevent item drops/duplicates: snapshot the loadout to a survivor bot, then strip items.
		RemoveAllItems( true );
	}
	else
	{
	...
	// If a living human survivor leaves the survivor team, replace them immediately with a bot
	// at the same position/health/loadout so the character stays in the world.
	if ( bShouldSurvivorBotTakeover )
	{
		CCSBot *bot = CreateSurvivorBotTakeover();
		if ( bot )
		{
			ApplySurvivorBotTakeoverSnapshot( bot, survivorTakeover );
		}
	}

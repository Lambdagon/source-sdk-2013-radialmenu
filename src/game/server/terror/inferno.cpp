...
IMPLEMENT_SERVERCLASS_ST( CInferno, DT_Inferno )
	...
	SendPropInt( SENDINFO( m_nInfernoType ), 3, SPROP_UNSIGNED ),
END_SEND_TABLE()
...
ConVar InfernoFlameLifetime( "inferno_flame_lifetime", "15", FCVAR_CHEAT, "Average lifetime of each flame in seconds" );
...
CInferno::CInferno()
{
	...
	m_pSpitterSpitMusic = NULL;
	m_bSpitterSpitMusicFading = false;
}
...
CInferno::~CInferno()
{
	...
	case INFERNO_TYPE_SPITTER_ACID:
		EmitSound( "SpitterZombie.Acid.fadeout" );
		StopSound( "SpitterZombie.Acid" );
		break;
	}		

	if ( m_pSpitterSpitMusic )
	{
		CSoundEnvelopeController::GetController().SoundDestroy( m_pSpitterSpitMusic );
		m_pSpitterSpitMusic = NULL;
	}
}
...
void CInferno::Precache( void )
{
	...
	// Spitter acid assets (inferno type INFERNO_TYPE_SPITTER_ACID).
	PrecacheScriptSound( "SpitterZombie.Acid" );
	PrecacheScriptSound( "SpitterZombie.Acid.fadeout" );
	PrecacheScriptSound( "Event.SpitterSpit" );
	PrecacheScriptSound( "Event.SpitterBurn" );
	PrecacheParticleSystem( "spitter_areaofdenial" );
...
//------------------------------------------------------------------------------------------
int CInferno::GetDamageType()
{
	if ( GetInfernoType() == INFERNO_TYPE_SPITTER_ACID )
	{
		return DMG_ACID;
	}
...
//------------------------------------------------------------------------------------------
float CInferno::GetFlameSpreadDelay()
{
	// Default behavior: spread continuously.
	return 0.0f;
}
...
void CInferno::StartBurning( const Vector &pos, const Vector &normal, const Vector &velocity, int initialDepth )
{
	...
	if ( k_ECreateFireResult_OK == CreateFire( m_startPos, normal, NULL, initialDepth ) )
	{
		...
		case INFERNO_TYPE_FIREWORKS:
			...
			break;
		case INFERNO_TYPE_SPITTER_ACID:
			EmitSound( "SpitterZombie.Acid" );
			{
				CTeamRecipientFilter filter( TEAM_SURVIVOR );
				m_pSpitterSpitMusic = CSoundEnvelopeController::GetController().SoundCreate( filter, entindex(), CHAN_STATIC, "Event.SpitterSpit", ATTN_NONE );
				if ( m_pSpitterSpitMusic )
				{
					CSoundEnvelopeController::GetController().Play( m_pSpitterSpitMusic, 1.0f, 100 );
				}
			}
			break;
		}
...
void CInferno::Spread( const Vector &spreadVelocity )
{
	...
	for ( int i = 0; i < m_fireCount && traceCount > 0; i++ )
	{
		...
		for( int t=0; t<maxRetry; ++t )
		{
			...
			Vector timeAdjustedSpreadVelocity = spreadVelocity * fire->m_lifetime.GetRemainingTime() * velocityDecay;
...
bool CInferno::IsFirePosInSmokeCloud( const Vector &pos ) const
{
	...
	int iNumEntities = UTIL_EntitiesInSphere( pEntities, INFERNO_SEARCH_ENTS, pos, 155, FL_GRENADE );
...
void CInferno::InfernoThink( void )
{
	...
	// Fade out the spit music as the acid expires so it matches the fire "going out" visuals.
	if ( GetInfernoType() == INFERNO_TYPE_SPITTER_ACID && m_pSpitterSpitMusic && !m_bSpitterSpitMusicFading && m_activeTimer.HasStarted() )
	{
		const float fadeTime = 1.0f;
		const float remaining = GetFlameLifetime() - m_activeTimer.GetElapsedTime();
		if ( remaining <= fadeTime )
		{
			m_bSpitterSpitMusicFading = true;
			CSoundEnvelopeController::GetController().SoundFadeOut( m_pSpitterSpitMusic, fadeTime, true );
			m_pSpitterSpitMusic = NULL;
		}
	}
	...
	while ( m_damageTimer.RunEvery( kDamageTimerSeconds ) )
	{
		...
		const bool isSpitterAcid = ( GetInfernoType() == INFERNO_TYPE_SPITTER_ACID );
		...
		for( int i=0; i<count; ++i )
		{
			...
			if (IsTouching( list[i], flameRadius, list[i]->IsPlayer() ))
			{
				if ( isSpitterAcid )
				{
					CBaseMultiplayerPlayer *pCS = dynamic_cast< CBaseMultiplayerPlayer * >( list[i] );
					if ( !pCS || pCS->GetTeamNumber() != TEAM_SURVIVOR )
					{
						continue;
					}
				}
		...
		for ( int i = 0; i < damageCount; i++ )
		{
			...
			if( CanHarm( pEnt ) )
			{
				if ( isSpitterAcid )
				{
					CBaseMultiplayerPlayer *pVictim = dynamic_cast< CBaseMultiplayerPlayer * >( pEnt );
					if ( !pVictim || pVictim->GetTeamNumber() != TEAM_SURVIVOR )
					{
						continue;
					}

					int foundIndex = -1;
					for ( int j = 0; j < m_acidVictims.Count(); ++j )
					{
						if ( m_acidVictims[j].hVictim.Get() == pVictim )
						{
							foundIndex = j;
							break;
						}
					...
					if ( foundIndex < 0 )
					{
						AcidVictimInfo_t info;
						info.hVictim = pVictim;
						info.flEnterTime = gpGlobals->curtime;
						info.flLastTouchTime = gpGlobals->curtime;
						info.bSpokeGooed = false;
						foundIndex = m_acidVictims.AddToTail( info );
					}

					AcidVictimInfo_t &info = m_acidVictims[ foundIndex ];
					info.flLastTouchTime = gpGlobals->curtime;

					// Speak once when first damaged by this acid pool.
					if ( !info.bSpokeGooed )
					{
						info.bSpokeGooed = true;
						pVictim->SpeakConceptIfAllowed( GetMPConceptIndexFromString( "GooedBySpitter" ) );
					}

					const float elapsed = MAX( 0.0f, gpGlobals->curtime - info.flEnterTime );
					const int secondsIn = (int)floorf( elapsed );
					const float dps = 2.0f + 2.0f * (float)secondsIn;
					if (isSpitterAcid)
					{
						damage = 4;
					}
					else {
						damage = dps * kDamageTimerSeconds;
					}
					damageType = DMG_ACID;

					if ( CCSPlayer *pCSVictim = dynamic_cast< CCSPlayer * >( pVictim ) )
					{
						pCSVictim->NotifySpitterAcidBurned();
					}
				}
		...
		// Remove victims who have left the acid.
		if ( isSpitterAcid )
		{
			const float pruneAfter = kDamageTimerSeconds + 0.05f;
			for ( int j = m_acidVictims.Count() - 1; j >= 0; --j )
			{
				if ( !m_acidVictims[j].hVictim.Get() || ( gpGlobals->curtime - m_acidVictims[j].flLastTouchTime ) > pruneAfter )
				{
					m_acidVictims.FastRemove( j );
				}
			}
		}
	}

...
public:
	...
	void ComputeNewMarineFacing( CTFPlayer *pPlayer, const Vector &HitLocation, C_BaseEntity *pHitEnt, IASW_Client_Aim_Target *pAutoAimEnt, bool bPreferFlatAiming, float *pPitch, Vector *pNewMarineFacing );
	...
	void ASW_GetCameraLocation( CTFPlayer *pPlayer, Vector &vecCameraLocation, QAngle &angCamera, int &nMouseX, int &nMouseY, bool bApplySmoothing );
...
private:
	...
	void CalculateCameraShift( CTFPlayer *pPlayer, float flDeltaX, float flDeltaY, float &flShiftX, float &flShiftY );
	void SmoothCamera( CTFPlayer *pPlayer, Vector &vecCameraLocation );

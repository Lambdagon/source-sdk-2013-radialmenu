...
private:
	...
	CPanelAnimationVarAliasType( float, m_flLargeBoxWide, "LargeBoxWide", "32", "proportional_float" );
	CPanelAnimationVarAliasType( float, m_flLargeBoxTall, "LargeBoxTall", "32", "proportional_float" );

	CPanelAnimationVarAliasType( float, m_flBoxGap, "BoxGap", "12", "proportional_float" );
	CPanelAnimationVarAliasType( float, m_flRightMargin, "RightMargin", "24", "proportional_float" );
	CPanelAnimationVarAliasType( float, m_flTopMargin, "TopMargin", "0", "proportional_float" );
	...
	CPanelAnimationVarAliasType( float, m_flIconXPos, "IconXPos", "0", "proportional_float" );
	CPanelAnimationVarAliasType( float, m_flIconYPos, "IconYPos", "0", "proportional_float" );

	CPanelAnimationVarAliasType( float, m_flTextYPos, "TextYPos", "0", "proportional_float" );
	...
	CPanelAnimationVar( float, m_flAutoHideTime, "AutoHideTime", "3.0" );
};
...
CHudWeaponSelection::CHudWeaponSelection( const char *pElementName ) : CBaseHudWeaponSelection(pElementName), BaseClass(NULL, "HudWeaponSelection")
{
	...
	// Never eat mouse input; this HUD is purely informational.
...
void CHudWeaponSelection::ProcessInput()
{
	// This HUD is informational only. Never consume mouse buttons for weapon selection.
}
...
void CHudWeaponSelection::Paint()
{
	...
	int xRight = MAX( 0, GetWide() - (int)m_flRightMargin );
	int ypos = MAX( 0, (int)m_flTopMargin );
	...
	for ( int i = 0; i < m_iMaxSlots; i++ )
	{
		if ( i == iActiveSlot )
		{
			...
			for (int slotpos = 0; slotpos < MAX_WEAPON_POSITIONS; slotpos++)
			{
				...
				int xpos = xCursorRight - largeBoxWide;


				// draw icon
				Color col = GetFgColor();
				// icons use old system, drawing in screen space
				const bool bSelectedWeapon = ( pWeapon == pSelectedWeapon );
				const CHudTexture *pIcon = bSelectedWeapon ? pWeapon->GetSpriteActive() : pWeapon->GetSpriteInactive();
				if ( pIcon && pIcon->bRenderUsingFont )
				{
					pIcon = NULL;
				}
				if ( !pIcon )
				{
					pIcon = bSelectedWeapon ? pWeapon->GetSpriteInactive() : pWeapon->GetSpriteActive();
					if ( pIcon && pIcon->bRenderUsingFont )
					{
						pIcon = NULL;
					}
				...
				if ( pIcon )
				{
					...
					pIcon->DrawSelf((xpos - pIcon->Width()) + 64, ypos, col );
					// draw selected weapon
					DrawBox((xpos - pIcon->Width()) + 64, ypos, pIcon->Width(), pIcon->Height(), selectedColor, m_flSelectionAlphaOverride, bFirstItem ? i + 1 : -1);
				}
				...
				xCursorRight -= (largeBoxWide + m_flBoxGap);
				bFirstItem = false;
			}
		...
		else
		{
			...
			int xpos = xRight - m_flSmallBoxSize;
			C_BaseCombatWeapon *pFirstWeaponInSlot = GetFirstPos( i );
			if ( pFirstWeaponInSlot )
			{
				// draw has weapon in slot

				// also draw the weapon icon for inactive slots
				const CHudTexture *pIcon = pFirstWeaponInSlot->GetSpriteInactive();
				if ( pIcon && pIcon->bRenderUsingFont )
				{
					pIcon = NULL;
				}
				if ( !pIcon )
				{
					pIcon = pFirstWeaponInSlot->GetSpriteActive();
					if ( pIcon && pIcon->bRenderUsingFont )
					{
						pIcon = NULL;
					}
				...
				if ( pIcon )
				{
					...
					if ( !pFirstWeaponInSlot->CanBeSelected() )
					{
						iconColor = Color( 255, 0, 0, iconColor[3] );
					}
					else
					{
						iconColor[3] = m_flAlphaOverride;
					}

					DrawBox(((xpos - pIcon->Width()) + 64), ypos, pIcon->Width(), pIcon->Height(), m_BoxColor, m_flAlphaOverride, i + 1);
					const int boxSize = (int)m_flSmallBoxSize;
					const int pad = MAX( 1, boxSize / 8 );
					pIcon->DrawSelf( ((xpos - pIcon->Width() + pad) + 64), ypos + pad, pIcon->Width() - ( pad * 2 ), pIcon->Height() - ( pad * 2 ), iconColor );
				}
			}
			else
			{
...
void CHudWeaponSelection::SelectWeapon( void )
{
	...
	// Keep the HUD visible briefly after switching (TF2-style).
	if ( !IsInSelectionMode() )
	{
		OpenSelection();
	}

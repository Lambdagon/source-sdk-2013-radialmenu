...
bool ClientModeCSNormal::ShouldDrawLocalPlayer( C_BasePlayer *pPlayer )
{
	// Always allow the view entity/player to be added to the visible entities list.
	// This lets the local player's world model render even in first-person view.
	return true;
}
...
int	ClientModeCSNormal::KeyInput( int down, ButtonCode_t keynum, const char *pszCurrentBinding )
{
	...
	// FIXME: this breaks the bots -siob
	//if( CSGameRules() && CSGameRules()->IsLogoMap() )
		//return 1;

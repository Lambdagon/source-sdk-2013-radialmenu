...
IMPLEMENT_CLIENTCLASS_DT( C_Inferno, DT_Inferno, CInferno )
	...
	RecvPropInt( RECVINFO( m_nInfernoType ) ),
END_RECV_TABLE()
...
void C_Inferno::Spawn( void )
{
	...
	m_drawableCount = 0;
	const char *pszEffect = GetParticleEffectName();
	m_burnParticleEffect = ParticleProp()->Create( pszEffect ? pszEffect : "molotov_groundfire", PATTACH_ABSORIGIN_FOLLOW );
	m_nLastInfernoType = m_nInfernoType;
...
void C_Inferno::ClientThink()
{
	...
	if ( m_nLastInfernoType != m_nInfernoType )
	{
		if ( m_burnParticleEffect )
		{
			m_burnParticleEffect->StopEmission();
			m_burnParticleEffect = NULL;
		}
...
void C_Inferno::UpdateParticles( void )
{
	if ( m_drawableCount > 0 && (InfernoFire.GetInt() & NEW_FIRE_MASK) != 0 )
	{
			for( int i=0; i<m_drawableCount && i<MAX_PARTICLE_CONTROL_POINTS; ++i )
			{
				...
				{
					if (!m_burnParticleEffect) return;
